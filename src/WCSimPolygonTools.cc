...
//   	
	...
	double mSide = (vertY.at((whichSide + 1) % nSides) - vertY.at(whichSide))
			/ (vertX.at((whichSide + 1) % nSides) - vertX.at(whichSide));
...
bool PolygonContainsSquare(unsigned int nSides, double outerRadius, G4TwoVector squareCorner, double squareSide) {
	...
	for (std::vector<G4TwoVector>::const_iterator itr = squareCorners.begin(); itr != squareCorners.end(); ++itr) {
...
//			std::cout << "Difference > 2pi for " << startAngle << " and " << endAngle << std::endl;
	...
	if (normEnd < normStart) {
		normEnd += twoPi;
	} else if ((normEnd - normStart) >= twoPi) {
		normEnd -= twoPi;
	}
