...
void WCSimTrackingAction::PreUserTrackingAction(const G4Track *aTrack)
{
	...
	if (aTrack->GetDefinition() != G4OpticalPhoton::OpticalPhotonDefinition() || G4UniformRand() < fFractionCherenkovPhotonsToDraw)
	{
...
void WCSimTrackingAction::PostUserTrackingAction(const G4Track *aTrack)
{
	...
	if (aTrack->GetParentID() == 0 || ((creatorProcess != 0) && ProcessList.count(creatorProcess->GetProcessName())) || (ParticleList.count(aTrack->GetDefinition()->GetPDGEncoding())) || (aTrack->GetDefinition()->GetPDGEncoding() == 22 && aTrack->GetTotalEnergy() > 50.0 * CLHEP::MeV) || (aTrack->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()))
	{
...
void WCSimTrackingAction::SetFractionCherenkovPhotonsToDraw(const double &frac)
{
	...
	if (fFractionCherenkovPhotonsToDraw > 1.0)
	{
		fFractionCherenkovPhotonsToDraw = 1.0;
	}
	else if (fFractionCherenkovPhotonsToDraw < 0.0)
	{
		fFractionCherenkovPhotonsToDraw = 0.0;
	}
}
