...
bool WCSimDetectorZone::IsGood(WCSimGeometryEnums::DetectorRegion_t region) const
{
	...
	std::cerr << "Must have the same number of direction types as names ("
	<< fDirectionType.size() << " vs " << fCellPMTName.size() << std::endl;
	isGood = false;
	}
	if( fCellDirTheta.size() != fCellPMTName.size()){
	std::cerr << "Must have the same number of PMT theta directions as names ("
	<< fCellDirTheta.size() << " vs " << fCellPMTName.size() << std::endl;
	isGood = false;
	}
	if( fCellDirPhi.size() != fCellPMTName.size()){
	std::cerr << "Must have the same number of PMT phi directions as names ("
	<< fCellDirPhi.size() << " vs " << fCellPMTName.size() << std::endl;
	isGood = false;
	}
...
void WCSimDetectorZone::Print() const
{
  ...
  for(unsigned int iPMT = 0; iPMT < fCellPMTName.size(); ++iPMT)
  {
      ...
      std::cout << "\t\t\t" << "fDirectionType = " << fDirectionType.at(iPMT).AsString() << " facing (" << fCellDirTheta.at(iPMT) << "," << fCellDirPhi.at(iPMT) << ")" << std::endl << std::endl;
  }
}
...
WCSimGeometryEnums::PMTDirection_t WCSimDetectorZone::GetCellPMTFaceType(
...
int WCSimDetectorZone::GetMaxNumCells() const
{
  ...
  for( ; limitItr != fPMTLimit.end(); ++limitItr)
  {
  	...
  	if( maxCells == 0 || numCells < maxCells ){ maxCells = numCells; }
  }
