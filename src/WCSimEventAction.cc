...
WCSimEventAction::WCSimEventAction(WCSimRunAction *myRun, WCSimDetectorConstruction *myDetector,
								   WCSimPrimaryGeneratorAction *myGenerator) : runAction(myRun), generatorAction(myGenerator), detectorConstructor(myDetector)
{
...
void WCSimEventAction::EndOfEventAction(const G4Event *evt)
{
	...
	if (GetRunAction()->GetSavePhotonNtuple() || GetRunAction()->GetSaveEmissionProfile())
	{
		...
		for (G4int i = 0; i < n_trajectories; i++)
		{
			...
			{
				WCSimPhotonNtuple::Fill(event_id, pdgCode, trackID, parentID, processID, energy, momentum, lambda,
										opticalPhoton, scatteredPhoton, vtxX, vtxY, vtxZ, vtxTime, endX, endY, endZ, endTime, vtxdirX,
										vtxdirY, vtxdirZ);
			}
		}
	}
	...
	{
		FillRootEvent(event_id, trajectoryContainer, WCHC, WCDC);
	}
}
...
G4int WCSimEventAction::WCSimEventFindStartingVolume(G4ThreeVector vtx)
{
	...
	if (vtxVolumeName == "outerTube" || vtxVolumeName == "innerTube" || vtxVolumeName == "rearEndCap" || vtxVolumeName == "frontEndCap")
	{
		vtxvol = 10;
	}
	else if (vtxVolumeName.contains("WC") && !vtxVolumeName.contains("FV"))
	{
		...
		if ((vtxVolumeName.contains("WCBarrel")) || (vtxVolumeName.contains("Tank")))
		{ //aah I added "Tank" as MB equivalent of Barrel
			vtxvol = 10;
		}
		...
		else if (vtxVolumeName.contains("PMT") || vtxVolumeName.contains("Cap") || vtxVolumeName.contains("Cell"))
		{
			vtxvol = 11;
		}
...
G4int WCSimEventAction::WCSimEventFindStoppingVolume(G4String stopVolumeName)
{
	...
	if (stopVolumeName.contains("WC") && !stopVolumeName.contains("FV"))
	{
		...
		if ((stopVolumeName.contains("WCBarrel")) || (stopVolumeName.contains("Tank"))) // aah I added "Tank" as MB equivalent of Barrel
			...
			stopvol = -2;
		else if (stopVolumeName.contains("PMT") || stopVolumeName.contains("Cap") || stopVolumeName.contains("Cell"))
			stopvol = 11;
...
void WCSimEventAction::FillRootEvent(G4int event_id, G4TrajectoryContainer *TC, WCSimWCHitsCollection *WCHC,
									 WCSimWCDigitsCollection *WCDC)
{
	...
	for (unsigned int i = 0; i < 3; ++i)
	{
		dir[i] = beamDir[i];
		pdir[i] = beamMom[i];
		stop[i] = vtx[i] / CLHEP::cm; // Apparently not meaningful?
		start[i] = 0.0;				  // I never saw this set anywhere.
	}

	wcsimrootevent->AddTrack(truthSum.GetBeamPDG(), -1, // This is the dummy flag for the neutrino
	...
	if (targetPDG != 0 && targetPDG != -999)
	{ // protects against seg-fault
		if (targetPDG > 999)
		{							   // 16O nucleus not in pdg table
	...
	if (targetPDG != -999)
	{
		wcsimrootevent->AddTrack(targetPDG, -2, // This is the dummy flag for the target
								 targetMass,	// Mass
								 targetPMag,	// Momentum
								 targetEnergy,	// Energy
								 -1,			// Start volume = -1
		...
		if (trj->GetSaveFlag())
		{
			...
			if (!((ipnu == 22) && (parentType == 999)))
			{
				...
				if (ngates)
				{
					...
					if (ttime > WCDM->GetTriggerTime(0) + 950. && WCDM->GetTriggerTime(1) + 950. > ttime)
						choose_event = 1;
					if (ttime > WCDM->GetTriggerTime(1) + 950. && WCDM->GetTriggerTime(2) + 950. > ttime)
						choose_event = 2;
				...
				wcsimrootevent->AddTrack(ipnu, flag, mass, mommag, energy, startvol, stopvol, dir, pdir, stop, start,
										 parentType, ttime, id, parentID);
			}
	...
	if (WCHC)
	{
		...
		for (int k = 0; k < WCHC->entries(); k++)
		{
			...
			wcsimrootevent->AddCherenkovHit(tubeID, truetime, primaryParentID);
		}
	}
	...
	if (WCDC)
	{
		...
		for (int index = 0; index < ngates; index++)
		{
			...
			for (int k = 0; k < WCDC->entries(); k++)
			{
				...
				{
					wcsimrootevent->AddCherenkovDigiHit((*WCDC)[k]->GetPe(index), (*WCDC)[k]->GetTime(index),
														(*WCDC)[k]->GetTubeID());
