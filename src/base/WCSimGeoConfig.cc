...
bool WCSimGeoConfig::IsGood() const
{
	...
	for (std::vector<WCSimGeometryEnums::DetectorRegion_t>::const_iterator regItr = regions.begin();
		 regItr != regions.end(); ++regItr)
	{
		for (unsigned int i = 0; i < GetNumZones(*regItr); ++i)
		{
			...
			{
				std::cerr << "Error: could not find entry for region " << (*regItr).AsString() << " zone " << i
						  << std::endl;
				isGood = false;
			}
			else
	...
	if (!isGood)
	{
		std::cerr << "Check the geometry xml file!" << std::endl;
	}
	return isGood;
}
...
bool WCSimGeoConfig::IsGoodZone(std::pair<WCSimGeometryEnums::DetectorRegion_t, int> zone) const
{
	...
	std::map<std::pair<WCSimGeometryEnums::DetectorRegion_t, int>, WCSimDetectorZone>::const_iterator zoneItr =
		fZoneMap.find(zone);
...
void WCSimGeoConfig::Print() const
{
	...
	std::cout << "\tOuter Radius = " << this->GetOuterRadius() / CLHEP::m << "m" << std::endl;
	std::cout << "\tInner Height = " << this->GetInnerHeight() / CLHEP::m << "m" << std::endl;
	...
	for (std::map<std::string, int>::const_iterator limItr = fPMTLimitMap.begin(); limItr != fPMTLimitMap.end();
		 ++limItr)
	{
	...
	{
		std::cout << "\t\t Region is " << (*zoneItr).first.first.AsString() << " and zone number = "
				  << (*zoneItr).first.second << std::endl;
...
void WCSimGeoConfig::AddCurrentRegion(std::string regionName)
{
	...
	std::vector<WCSimGeometryEnums::DetectorRegion_t>::iterator regionItr = std::find(fCurrentRegions.begin(),
																					  fCurrentRegions.end(), region);
...
void WCSimGeoConfig::AddCellPMTName(WCSimGeometryEnums::DetectorRegion_t region, int zone, std::string name)
{
	CreateMissingZone(region, zone);
	fZoneMap[std::make_pair(region, zone)].AddCellPMTName(name);
}

std::string WCSimGeoConfig::GetCellPMTName(WCSimGeometryEnums::DetectorRegion_t region, int zone,
										   unsigned int pmt) const
{
	std::string name("");
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zone);
	std::map<std::pair<WCSimGeometryEnums::DetectorRegion_t, int>, WCSimDetectorZone>::const_iterator zoneItr =
		fZoneMap.find(myPair);
	if (zoneItr != fZoneMap.end())
	{
	...
	{
		std::cerr << "Error: Could not find PMT number " << pmt << " in region " << region.AsString() << ", zone "
				  << zone << std::endl;
		assert(0);
	}
	return name;
...
std::vector<std::string> WCSimGeoConfig::GetCellPMTName(WCSimGeometryEnums::DetectorRegion_t region, int zone) const
{
...
void WCSimGeoConfig::AddCellPMTX(WCSimGeometryEnums::DetectorRegion_t region, int zone, double x)
{
	CreateMissingZone(region, zone);
	fZoneMap[std::make_pair(region, zone)].AddCellPMTX(x);
}

double WCSimGeoConfig::GetCellPMTX(WCSimGeometryEnums::DetectorRegion_t region, int zone, unsigned int pmt) const
{
	double x = -999.9;
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zone);
	std::map<std::pair<WCSimGeometryEnums::DetectorRegion_t, int>, WCSimDetectorZone>::const_iterator zoneItr =
...
std::vector<double> WCSimGeoConfig::GetCellPMTX(WCSimGeometryEnums::DetectorRegion_t region, int zone) const
{
...
void WCSimGeoConfig::AddCellPMTY(WCSimGeometryEnums::DetectorRegion_t region, int zone, double y)
{
	CreateMissingZone(region, zone);
	fZoneMap[std::make_pair(region, zone)].AddCellPMTY(y);
}

double WCSimGeoConfig::GetCellPMTY(WCSimGeometryEnums::DetectorRegion_t region, int zone, unsigned int pmt) const
{
	double y = -999.9;
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zone);
	std::map<std::pair<WCSimGeometryEnums::DetectorRegion_t, int>, WCSimDetectorZone>::const_iterator zoneItr =
...
std::vector<double> WCSimGeoConfig::GetCellPMTY(WCSimGeometryEnums::DetectorRegion_t region, int zone) const
{
...
void WCSimGeoConfig::AddCellPMTFaceType(WCSimGeometryEnums::DetectorRegion_t region, int zone,
										WCSimGeometryEnums::PMTDirection_t type)
{
	CreateMissingZone(region, zone);
	fZoneMap[std::make_pair(region, zone)].AddCellPMTFaceType(type);
}

double WCSimGeoConfig::GetCellPMTFaceType(WCSimGeometryEnums::DetectorRegion_t region, int zone,
										  unsigned int pmt) const
{
...
void WCSimGeoConfig::AddCellPMTFaceTheta(WCSimGeometryEnums::DetectorRegion_t region, int zone, double theta)
{
	CreateMissingZone(region, zone);
	fZoneMap[std::make_pair(region, zone)].AddCellPMTFaceTheta(theta);
}

double WCSimGeoConfig::GetCellPMTFaceTheta(WCSimGeometryEnums::DetectorRegion_t region, int zone,
										   unsigned int pmt) const
{
	double theta = -999.9;
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zone);
...
std::vector<double> WCSimGeoConfig::GetCellPMTFaceTheta(WCSimGeometryEnums::DetectorRegion_t region, int zone) const
{
...
void WCSimGeoConfig::AddCellPMTFacePhi(WCSimGeometryEnums::DetectorRegion_t region, int zone, double phi)
{
	CreateMissingZone(region, zone);
	fZoneMap[std::make_pair(region, zone)].AddCellPMTFacePhi(phi);
}

double WCSimGeoConfig::GetCellPMTFacePhi(WCSimGeometryEnums::DetectorRegion_t region, int zone,
										 unsigned int pmt) const
{
	double phi = -999.9;
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zone);
...
std::vector<double> WCSimGeoConfig::GetCellPMTFacePhi(WCSimGeometryEnums::DetectorRegion_t region, int zone) const
{
...
void WCSimGeoConfig::SetZonalCoverage(WCSimGeometryEnums::DetectorRegion_t region, int zone, double coverage)
{
	CreateMissingZone(region, zone);
...
void WCSimGeoConfig::SetPMTLimit(WCSimGeometryEnums::DetectorRegion_t region, int zone, std::string name, int limit)
{
	CreateMissingZone(region, zone);
	fZoneMap[std::make_pair(region, zone)].SetPMTLimit(name, limit);
}
int WCSimGeoConfig::GetMaxZoneCells(WCSimGeometryEnums::DetectorRegion_t region, int zone) const
{
	int nCells = -1;
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zone);
	std::map<std::pair<WCSimGeometryEnums::DetectorRegion_t, int>, WCSimDetectorZone>::const_iterator zoneItr =
...
void WCSimGeoConfig::SetZoneThetaStart(WCSimGeometryEnums::DetectorRegion_t region, int zoneNum, double theta)
{
...
void WCSimGeoConfig::SetZoneThetaEnd(WCSimGeometryEnums::DetectorRegion_t region, int zoneNum, double theta)
{
	CreateMissingZone(region, zoneNum);
	if (region == WCSimGeometryEnums::DetectorRegion_t::kTop || region == WCSimGeometryEnums::DetectorRegion_t::kBottom)
	{
...
double WCSimGeoConfig::GetZoneThetaStart(WCSimGeometryEnums::DetectorRegion_t region, int zoneNum)
{
...
double WCSimGeoConfig::GetZoneThetaEnd(WCSimGeometryEnums::DetectorRegion_t region, int zoneNum)
{
	double thetaEnd = 0.0;
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zoneNum);
	std::map<std::pair<WCSimGeometryEnums::DetectorRegion_t, int>, WCSimDetectorZone>::const_iterator zoneItr =
...
unsigned int WCSimGeoConfig::GetNumZones(WCSimGeometryEnums::DetectorRegion_t region) const
{
...
void WCSimGeoConfig::SetCoverageType(WCSimGeometryEnums::PhotodetectorLimit_t type)
{
	// std::cout << "Setting coverage type to " << type.AsString() << std::endl;
	if (type == WCSimGeometryEnums::PhotodetectorLimit_t::kPercentCoverage)
	{
		SetUseOverallCoverage(true);
	}
	else if (type == WCSimGeometryEnums::PhotodetectorLimit_t::kZonalCoverage)
	{
		SetUseZonalCoverage(true);
	}
	else if (type == WCSimGeometryEnums::PhotodetectorLimit_t::kTotalNumber)
	{
		SetLimitPMTNumbers(true);
	}
	else
	{
...
void WCSimGeoConfig::CreateMissingZone(WCSimGeometryEnums::DetectorRegion_t region, int zone)
{
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zone);
...
{
	if (fPMTLimit != WCSimGeometryEnums::PhotodetectorLimit_t::kUnknown && fPMTLimit != WCSimGeometryEnums::PhotodetectorLimit_t::kTotalNumber && doIt)
	{
		std::cerr << "Error: can't set photodetector limit to fixed PMT number - is already set to "
				  << fPMTLimit.AsString();
	}
	if (doIt)
	{
...
void WCSimGeoConfig::SetPMTLimit(std::string name, int limit)
{
	...
	{
		std::cerr << "Error: Cannot fix PMT numbers, because the limit type is currently " << fPMTLimit.AsString()
				  << std::endl;
...
{
	if (fPMTLimit != WCSimGeometryEnums::PhotodetectorLimit_t::kUnknown && fPMTLimit != WCSimGeometryEnums::PhotodetectorLimit_t::kPercentCoverage && doIt)
	{
		std::cerr << "Error: can't set photodetector limit to a fixed global coverage - is already set to "
				  << fPMTLimit.AsString();
	}
	if (doIt)
	{
...
{
	if (fPMTLimit != WCSimGeometryEnums::PhotodetectorLimit_t::kUnknown && fPMTLimit != WCSimGeometryEnums::PhotodetectorLimit_t::kZonalCoverage && doIt)
	{
		std::cerr << "Error: can't set photodetector limit to a fixed global coverage - is already set to "
				  << fPMTLimit.AsString();
	}
...
double WCSimGeoConfig::GetZonalCoverageFraction(WCSimGeometryEnums::DetectorRegion_t region, int zone) const
{
	double fraction = 0.0;
	std::pair<WCSimGeometryEnums::DetectorRegion_t, int> myPair = std::make_pair(region, zone);
	std::map<std::pair<WCSimGeometryEnums::DetectorRegion_t, int>, WCSimDetectorZone>::const_iterator zoneItr =
