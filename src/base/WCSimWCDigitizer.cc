...
WCSimWCDigitizer::WCSimWCDigitizer(G4String name, WCSimDetectorConstruction *myDet) : G4VDigitizerModule(name)
{
...
void WCSimWCDigitizer::FindNumberOfGatesFast()
{
	...
	for (_mGateKeeper = GateMap.begin(); _mGateKeeper != GateMap.end(); _mGateKeeper++)
	{
		...
		while (_mNextGate != GateMap.lower_bound(_mGateKeeper->first + offset200) && _mNextGate->first <= _mGateKeeper->first + offset200) // but not more than 200ns away though!
		{
...
void WCSimWCDigitizer::DigitizeGate(WCSimWCHitsCollection *WCHC, G4int G)
{
	...
	{
		lowerbound =
			(TriggerTimes[G] + EvtG8Down >= TriggerTimes[G - 1] + EvtG8Up) ? TriggerTimes[G] + EvtG8Down : TriggerTimes[G - 1] + EvtG8Up;
	}
	...
	for (G4int i = 0; i < WCHC->entries(); i++)
	{
		...
		G4float trueHitTime =
			(fDet->GetPMTTime() == 1) ? (*WCHC)[i]->GetMeanHitTimeInGate() : (*WCHC)[i]->GetFirstHitTimeInGate(lowerbound, upperbound);

		// Check to see if the hit is in the gate
		if (trueHitTime < 0.)
		{			  //PMT not hit in this gate
