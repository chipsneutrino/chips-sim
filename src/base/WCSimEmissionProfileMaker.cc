...
void WCSimEmissionProfileMaker::FillEvent(G4TrajectoryContainer *trajCont, WCSimTruthSummary *truth)
{
	...
	for (int i = 0; i < nTraj; ++i)
	{
		...
		if (trj->IsOpticalPhoton() && !trj->IsScatteredPhoton() && (abs(primaryPDG) != 13 || trj->GetParentID() == 1))
		{
			...
			double scale = (CLHEP::joule / CLHEP::MeV) * (CLHEP::nm / CLHEP::m) * (1.0 / (CLHEP::h_Planck * CLHEP::c_light));
			float wavelength = ((CLHEP::h_Planck * CLHEP::c_light) / (trj->GetEnergy())) / CLHEP::nm;
...
void WCSimEmissionProfileMaker::NormaliseG()
{
	...
	for (int iRow = 1; iRow <= fSCosThetaCoarse->GetYaxis()->GetNbins(); ++iRow)
	{
		...
		for (int jCol = 1; jCol <= fSCosThetaCoarse->GetXaxis()->GetNbins(); ++jCol)
		{
			...
			{
				fGCoarse->SetBinContent(jCol, iRow,
										(fSCosThetaCoarse->GetBinContent(jCol, iRow) / fSCosThetaCoarse->GetXaxis()->GetBinWidth(jCol)) / (sumTheta * sumWidth));
			}
		}
		for (int jCol = 1; jCol <= fSCosThetaFine->GetXaxis()->GetNbins(); ++jCol)
		{
			if (fSCosThetaFine->GetBinContent(jCol, iRow) > 0)
			{
				...
				fGFine->SetBinContent(jCol, iRow,
									  fSCosThetaFine->GetBinContent(jCol, iRow) / fSCosThetaFine->GetXaxis()->GetBinWidth(jCol) / (sumTheta * sumWidth));
			}
		}
	}
}
...
void WCSimEmissionProfileMaker::MakeHistograms()
{
	...
	fSCosThetaFine = new TH2F("fSCosThetaFine", "", fNBinsThetaFine, fThetaFineMin, fThetaFineMax, fNBinsS, fSMin,
							  fSMax);
	fSCosThetaCoarse = new TH2F("fSCosThetaCoarse", "", fNBinsThetaCoarse, fThetaCoarseMin, fThetaCoarseMax, fNBinsS,
								fSMin, fSMax);
	fWavelengths = new TH1F("fWavelengths", ";Wavelength (CLHEP::nm); Photons", 1000, 0, 1000);

	const int nSecBins = 41; // Need an odd number of bins to keep away from having 1/cos(pi/2) at a bin boundart
	double cosThetaBins[nSecBins + 1] = {-1.0, -0.75, -0.5, -0.25, -0.01, 0.01, 0.125, 0.250, 0.375, 0.50, 0.533,
										 0.567, 0.60, 0.633, 0.667, 0.7, 0.705, 0.71, 0.715, 0.72, 0.725, 0.73, 0.735, 0.74, 0.745, 0.75, 0.755,
										 0.76, 0.765, 0.77, 0.775, 0.78, 0.785, 0.79, 0.795, 0.80, 0.833, 0.867, 0.9, 0.933, 0.967, 1.0};
	...
	fSSecTheta = new TH2F("fSSecTheta", ";sec#theta;s (CLHEP::cm);Photons", nSecBins, secBins, 40, fSMin, fSMax);
	fSCosThetaCheckBinning = new TH2F("fSCosThetaCheckBinning", ";cos#theta;s (CLHEP::cm);photons", nSecBins, cosThetaBins, 40,
									  fSMin, fSMax);
}
