...
void WCSimDetectorZone::Print() const
{
	...
	{
		std::cout << "\t\t\t" << fCellPMTName.at(iPMT) << " at (" << fCellPMTX.at(iPMT) / CLHEP::m << ", "
				  << fCellPMTY.at(iPMT) / CLHEP::m << ")" << std::endl;
		std::cout << "\t\t\t"
				  << "fDirectionType = " << fDirectionType.at(iPMT).AsString() << " facing ("
				  << fCellDirTheta.at(iPMT) << "," << fCellDirPhi.at(iPMT) << ")" << std::endl
				  << std::endl;
	}
}
