...
std::string WCSimEvDisplay::ConvertTrueEventType() const
{
	...
	if (fTruthSummary != 0x0)
	{
		...
		if (fTruthSummary->IsCCEvent() && fTruthSummary->IsQEEvent())
			type = "CC QE";
		if (fTruthSummary->IsNCEvent() && fTruthSummary->IsQEEvent())
			type = "NC QE";
		// Resonant events
		if (fTruthSummary->IsCCEvent() && fTruthSummary->IsResEvent())
			type = "CC Res";
		if (fTruthSummary->IsNCEvent() && fTruthSummary->IsResEvent())
			type = "NC Res";
		// DIS events
		if (fTruthSummary->IsCCEvent() && fTruthSummary->IsDISEvent())
			type = "CC DIS";
		if (fTruthSummary->IsNCEvent() && fTruthSummary->IsDISEvent())
			type = "NC DIS";
		// Coherent events
		if (fTruthSummary->IsCCEvent() && fTruthSummary->IsCohEvent())
			type = "CC Coherent";
		if (fTruthSummary->IsNCEvent() && fTruthSummary->IsCohEvent())
			type = "NC Coherent";
		// Others
		if (fTruthSummary->IsNueElectronElasticEvent())
			type = "Nue Elastic";
		if (fTruthSummary->IsInverseMuonDecayEvent())
		...
		if (fTruthSummary->GetInteractionMode() == WCSimTruthSummary::kOther)
		{
			...
			for (unsigned int n = 0; n < fTruthSummary->GetNPrimaries(); ++n)
			{
				...
				if (fTruthSummary->IsNuEEvent() && fTruthSummary->GetPrimaryPDG(n) == 11)
					isCC = true;
				if (fTruthSummary->IsNuMuEvent() && fTruthSummary->GetPrimaryPDG(n) == 13)
					isCC = true;
				if (fTruthSummary->IsNuTauEvent() && fTruthSummary->GetPrimaryPDG(n) == 15)
					isCC = true;
				// Antineutrinos
				if (fTruthSummary->IsNuEBarEvent() && fTruthSummary->GetPrimaryPDG(n) == -11)
					isCC = true;
				if (fTruthSummary->IsNuMuBarEvent() && fTruthSummary->GetPrimaryPDG(n) == -13)
					isCC = true;
				if (fTruthSummary->IsNuTauBarEvent() && fTruthSummary->GetPrimaryPDG(n) == -15)
					isCC = true;
