...
G4float WCSimDetectorConstruction::GetPMTQE(G4float PhotonWavelength, G4int flag, G4float low_wl, G4float high_wl,
											G4float ratio)
{
	// XQ  08/17/10
...
G4LogicalVolume *WCSimDetectorConstruction::ConstructMailboxWC()
{
	...
	// (symmetric around the origin). The PMT spacing is determined automatically from the "WCPMTPercentCoverage" value. Currently the surface area of a hemisphere (2*CLHEP::pi*r^2) is used'
	...
	WCPosition = 0.;																					  //     aah    WCPosition is used as a depth offset in the Cylinder Detector. I set it to "0" here.
																										  //	G4double WC_ActiveLayer_Depth  = WCBlackSheetThickness+WCPMTRadius+0.01*CLHEP::m;             //add 1cm extra thickness just in case
	G4double WC_ActiveLayer_Depth = WCBlackSheetThickness + fPMTConfigs[0].GetRadius() + 0.01 * CLHEP::m; //add 1cm extra thickness just in case
	...
	G4cout << "Cavern Length= " << WC_MB_Cavern_length / CLHEP::m << "CLHEP::m,  Cavern_Width= " << WC_MB_Cavern_width / CLHEP::m
		   << "CLHEP::m, Tank Depth= " << WC_MB_Tank_depth / CLHEP::m << "CLHEP::m \n";
	...
	WC_MB_Fid_Width -= 2 * (WC_ActiveLayer_Depth - WCBlackSheetThickness);	// that it will fit the FV+"veto" with the blacksheet front face 0.5 CLHEP::m from the wall
	...
	G4GeometryManager::GetInstance()->SetWorldMaximumExtent(
		WC_MB_Fid_Length > WC_MB_Fid_Depth ? WC_MB_Fid_Length : WC_MB_Fid_Depth);
	G4cout << "Computed tolerance = " << G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / CLHEP::mm << " CLHEP::mm"
		   << G4endl;

	// Now make the geometry
	G4double WC_MB_Dome_Height = WC_MB_Dome_Height_fraction * WC_MB_Cavern_width; //
	G4Box *WC_MB_tank_H20 = new G4Box("Tank", WC_MB_Cavern_length / 2.0, WC_MB_Cavern_width / 2.0,
									  WC_MB_Tank_depth / 2.0); //	Note I will add a rectangular airgap above this.
	G4Box *WC_MB_Tank_Airgap_volume = new G4Box("Airgap", WC_MB_Cavern_length / 2.0, WC_MB_Cavern_width / 2.0,
												WC_MB_Tank_Airgap / 2.0); //This is airgap above water
	...
	G4double radius_ceiling = WC_MB_Cavern_width / (2 * sin(phi_ceiling)); //so this volume is not necessarily a half cylinder (phi=CLHEP::pi)
	G4cout << "phi" << phi_ceiling << "   radius= " << radius_ceiling / CLHEP::m << "CLHEP::m"
		   << "\n";
	G4cout << "dome extent" << CLHEP::pi - phi_ceiling << "    " << CLHEP::pi + phi_ceiling << "\n";
	G4Tubs *WC_MB_Cavern_ceiling_cyl = new G4Tubs("ceiling_cyl", 0.0, radius_ceiling, WC_MB_Cavern_length / 2.,
												  CLHEP::pi - phi_ceiling, 2 * phi_ceiling);
	G4Box *subtract_to_make_ceiling = new G4Box("subtract", radius_ceiling, radius_ceiling,
												1 + WC_MB_Cavern_length / 2.); ///a little bit longer since I seemed to have some extra stuff left over.
	G4SubtractionSolid *WC_MB_Cavern_ceiling = new G4SubtractionSolid("Ceiling", WC_MB_Cavern_ceiling_cyl,
																	  subtract_to_make_ceiling, 0, G4ThreeVector(+WC_MB_Dome_Height, 0., 0.)); //this is now ceiling, but it needs to be rotated +90 degrees around y axis
	G4Transform3D rotation_dome = G4RotateY3D(CLHEP::pi / 2.);
	G4double delta = -radius_ceiling + WC_MB_Dome_Height + WC_MB_Tank_Airgap / 2;
	G4cout << "delta= " << delta / CLHEP::m << "CLHEP::m \n";
	...
	G4UnionSolid *WC_MB_Tank_Airgap_volume_dome = new G4UnionSolid("Airgap and dome", WC_MB_Tank_Airgap_volume,
																   WC_MB_Cavern_ceiling, translate_dome * rotation_dome); //hopefully rotates first, then translates
	//**********************
	...
	G4LogicalVolume *logic_WC_Cavern = new G4LogicalVolume(WC_Cavern,
														   WCSimMaterialsBuilder::Instance()->GetMaterial("Air"), "Cavern", 0, 0, 0);
	...
	G4LogicalVolume *logic_WC_MB_tank_H20 = new G4LogicalVolume(WC_MB_tank_H20,
																WCSimMaterialsBuilder::Instance()->GetMaterial(water), "Tank", 0, 0, 0);

	//Make Fiducial Volume.
	G4Box *WC_MB_Fiducial = new G4Box("Fiducial Box", WC_MB_Fid_Length / 2., WC_MB_Fid_Width / 2.,
									  WC_MB_Fid_Depth / 2.);
	G4LogicalVolume *logic_WC_MB_Fiducial = new G4LogicalVolume(WC_MB_Fiducial,
																WCSimMaterialsBuilder::Instance()->GetMaterial(water), "Fiducial_Log", 0, 0, 0);
	...
	//	G4double WCPMT_surfacearea=2*CLHEP::pi*WCPMTRadius*WCPMTRadius;//this is the surface area of a hemisphere
	//	G4double WCPMT_crossarea=CLHEP::pi*WCPMTRadius*WCPMTRadius;// this is just the front face crossectional area---this is our standard definition
	G4double WCPMT_crossarea = CLHEP::pi * fPMTConfigs[0].GetRadius() * fPMTConfigs[0].GetRadius(); // this is just the front face crossectional area---this is our standard definition
	...
	G4LogicalVolume *logic_WC_PMT_Cell = new G4LogicalVolume(WC_PMT_Cell,
															 WCSimMaterialsBuilder::Instance()->GetMaterial(water), "logic_PMT_Cell", 0, 0, 0); //***replica
	G4Box *WC_Blacksheet_Cell = new G4Box("WC_BlackSheet_Cell", WC_MB_PMT_Spacing / 2, WC_MB_PMT_Spacing / 2,
										  WCBlackSheetThickness / 2.); //blacksheet material to fit in cell ***replica
	G4LogicalVolume *logic_WC_Blacksheet_Cell = new G4LogicalVolume(WC_Blacksheet_Cell,
																	WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "logic_WC_Blacksheet_Cell", 0, 0, 0); //***replica
	...
	G4cout << "PMT Length Offset (CLHEP::m) " << WC_MB_PMT_offset_L / CLHEP::m << "PMT Width Offset (CLHEP::m) " << WC_MB_PMT_offset_W / CLHEP::m
		   << "PMT Depth Offset (CLHEP::m) " << WC_MB_PMT_offset_D / CLHEP::m << "\n";

	//WC_Active_LxW---perp to Z axis

	G4Box *WC_Active_LxW = new G4Box("LxW", WC_MB_Fid_Length / 2 + WC_ActiveLayer_Depth,
									 WC_MB_Fid_Width / 2 + WC_ActiveLayer_Depth, WC_ActiveLayer_Depth / 2); //active volume with PMT's
	G4LogicalVolume *logic_WC_Active_LxW_minus = new G4LogicalVolume(WC_Active_LxW,
																	 WCSimMaterialsBuilder::Instance()->GetMaterial(water), "LxWActive_minus", 0, 0, 0);
	G4LogicalVolume *logic_WC_Active_LxW_plus = new G4LogicalVolume(WC_Active_LxW,
																	WCSimMaterialsBuilder::Instance()->GetMaterial(water), "LxWActive_plus", 0, 0, 0);
	...
	G4Box *WC_BlacksheetLxW = new G4Box("WC_BlackSheetLxW",
										WC_MB_Fid_Length / 2 + WC_ActiveLayer_Depth - WCBlackSheetThickness,
	...
	G4SubtractionSolid *WC_BlacksheetLxW_donut = new G4SubtractionSolid("WC_BlacksheetLxW_donut", WC_BlacksheetLxW,
																		WC_BlacksheetLxW_donuthole, 0, G4ThreeVector(0., 0., 0.)); //this is now the blacksheet that will suround the PMT volumes  ***replica
	//***replica

	G4LogicalVolume *logic_WC_BlacksheetLxW = new G4LogicalVolume(WC_BlacksheetLxW_donut,
																  WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "LagicBlacksheet_LxW", 0, 0, 0); //***replica

	//*********************************replica*******************************

	G4Box *WC_BlacksheetLxW_corner_L = new G4Box("WC_BlackSheetLxW_corner_L",
												 WC_MB_Fid_Length / 2 + WC_ActiveLayer_Depth, WCBlackSheetThickness / 2., WC_ActiveLayer_Depth / 2.); //Blacksheeet corner--to lite-tite the fiducial volume--This should already be oriented correctly
	G4LogicalVolume *logic_WC_BlacksheetLxW_corner_L = new G4LogicalVolume(WC_BlacksheetLxW_corner_L,
																		   WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "LagicBlacksheet_LxW", 0, 0, 0);
	G4Box *WC_BlacksheetLxW_corner_W = new G4Box("WC_BlackSheetLxW_corner_W", WCBlackSheetThickness / 2.,
												 WC_MB_Fid_Width / 2 + WC_ActiveLayer_Depth, WC_ActiveLayer_Depth / 2.); //Blacksheet corner--lite-tite the fiducial volume--This should already be oriented correctly
	G4LogicalVolume *logic_WC_BlacksheetLxW_corner_W = new G4LogicalVolume(WC_BlacksheetLxW_corner_W,
																		   WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "LagicBlacksheet_LxW", 0, 0, 0);
	...
	G4Box *LxW_PMT_Row = new G4Box("LxW_PMT_Row", WC_MB_NumPMT_L * WC_MB_PMT_Spacing / 2, WC_MB_PMT_Spacing / 2,
								   WC_ActiveLayer_Depth / 2.);
	G4LogicalVolume *logic_LxW_PMT_Row = new G4LogicalVolume(LxW_PMT_Row,
															 WCSimMaterialsBuilder::Instance()->GetMaterial(water), "logic_LxW_PMT_Row", 0, 0, 0);
	G4Box *LxW_PMT_Volume = new G4Box("LxW_PMT_Volume", WC_MB_NumPMT_L * WC_MB_PMT_Spacing / 2,
									  WC_MB_NumPMT_W * WC_MB_PMT_Spacing / 2, WC_ActiveLayer_Depth / 2); //this is entire PMT volume
	G4LogicalVolume *logic_LxW_PMT_Volume = new G4LogicalVolume(LxW_PMT_Volume,
																WCSimMaterialsBuilder::Instance()->GetMaterial(water), "logic_LxW_PMT_Volume", 0, 0, 0);

	G4VPhysicalVolume *physiWC_LxW_PMT_Row = new G4PVReplica("WC_PMT_Row", logic_WC_PMT_Cell, logic_LxW_PMT_Row, kXAxis,
															 WC_MB_NumPMT_L, WC_MB_PMT_Spacing, 0.);

	G4VPhysicalVolume *physiWC_LxW_PMT_Volume = new G4PVReplica("WC_LxW_PMT_Volume", logic_LxW_PMT_Row,
																logic_LxW_PMT_Volume, kYAxis, WC_MB_NumPMT_W, WC_MB_PMT_Spacing, 0.);
	...
	G4cout << "Coverage was calculated to be: "
		   << (WC_MB_NumPMT_L * WC_MB_NumPMT_W * WCPMT_crossarea / (WC_MB_Fid_Length * WC_MB_Fid_Width)) << "\n";
	...
	G4VPhysicalVolume *phys_WC_Active_LxW_blacksheet_minus = new G4PVPlacement(0,						  // no rotation
	...
	G4VPhysicalVolume *phys_WC_Active_LxW_blacksheet_plus = new G4PVPlacement(0,						// no rotation
																			  position_blacksheet_LxW,	// its position
																			  logic_WC_BlacksheetLxW,	// its logical volume
																			  "LW_Blacksheet",			// its name
																			  logic_WC_Active_LxW_plus, // its mother volume
	...
	new G4LogicalBorderSurface("WaterBS_LW_PolySurface", phys_WC_Active_LxW_minus, phys_WC_Active_LxW_blacksheet_minus,
							   WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	new G4LogicalBorderSurface("WaterBS_LW_PolySurface", phys_WC_Active_LxW_plus, phys_WC_Active_LxW_blacksheet_plus,
							   WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	...
	const G4Transform3D rotation_LxW_plus = G4RotateX3D(CLHEP::pi);							  //rotate to looking in -z direction (180 degrees) for top face
	...
	G4Box *WC_Active_WxD = new G4Box("WxD", WC_MB_Fid_Depth / 2, WC_MB_Fid_Width / 2 + WC_ActiveLayer_Depth,
									 WC_ActiveLayer_Depth / 2); //active volume--increase width to cover ends to seal with blacksheet
	G4LogicalVolume *logic_WC_Active_WxD_minus = new G4LogicalVolume(WC_Active_WxD,
																	 WCSimMaterialsBuilder::Instance()->GetMaterial(water), "WxDActive_minus", 0, 0, 0);
	G4LogicalVolume *logic_WC_Active_WxD_plus = new G4LogicalVolume(WC_Active_WxD,
																	WCSimMaterialsBuilder::Instance()->GetMaterial(water), "WxDActive_plus", 0, 0, 0);
	G4Box *WC_BlacksheetWxD = new G4Box("WC_BlackSheetWxD", WC_MB_Fid_Depth / 2,
										WC_MB_Fid_Width / 2 + WC_ActiveLayer_Depth - WCBlackSheetThickness / 2., WCBlackSheetThickness / 2.); //blacksheet material
	...
	G4SubtractionSolid *WC_BlacksheetWxD_donut = new G4SubtractionSolid("WC_BlacksheetWxD_donut", WC_BlacksheetWxD,
																		WC_BlacksheetWxD_donuthole, 0, G4ThreeVector(0., 0., 0.)); //this is now the blacksheet that will suround the PMT volumes  ***replica
	//***replica

	G4LogicalVolume *logic_WC_BlacksheetWxD = new G4LogicalVolume(WC_BlacksheetWxD_donut,
																  WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "LagicBlacksheet_WxD", 0, 0, 0); //***replica
	...
	G4Box *WC_BlacksheetWxD_corner = new G4Box("WC_BlackSheetWxD_corner", WC_MB_Fid_Depth / 2,
											   WCBlackSheetThickness / 2., WC_ActiveLayer_Depth / 2.); //Blacksheet corner---This should already be oriented correctly
	G4LogicalVolume *logic_WC_BlacksheetWxD_corner = new G4LogicalVolume(WC_BlacksheetWxD_corner,
																		 WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "LagicBlacksheet_WxD_corner", 0, 0, 0);
	...
	G4Box *WxD_PMT_Row = new G4Box("WxD_PMT_Row", WC_MB_NumPMT_D * WC_MB_PMT_Spacing / 2, WC_MB_PMT_Spacing / 2,
								   WC_ActiveLayer_Depth / 2.);
	G4LogicalVolume *logic_WxD_PMT_Row = new G4LogicalVolume(WxD_PMT_Row,
															 WCSimMaterialsBuilder::Instance()->GetMaterial(water), "logic_WxD_PMT_Row", 0, 0, 0);
	G4Box *WxD_PMT_Volume = new G4Box("WxD_PMT_Volume", WC_MB_NumPMT_D * WC_MB_PMT_Spacing / 2,
									  WC_MB_NumPMT_W * WC_MB_PMT_Spacing / 2, WC_ActiveLayer_Depth / 2); //this is entire PMT volume
	G4LogicalVolume *logic_WxD_PMT_Volume = new G4LogicalVolume(WxD_PMT_Volume,
																WCSimMaterialsBuilder::Instance()->GetMaterial(water), "logic_WxD_PMT_Volume", 0, 0, 0);

	G4VPhysicalVolume *physiWC_WxD_PMT_Row = new G4PVReplica("WC_PMT_Row", logic_WC_PMT_Cell, logic_WxD_PMT_Row, kXAxis,
															 WC_MB_NumPMT_D, WC_MB_PMT_Spacing, 0.);

	G4VPhysicalVolume *physiWC_WxD_PMT_Volume = new G4PVReplica("WC_PMT_Volume", logic_WxD_PMT_Row,
																logic_WxD_PMT_Volume, kYAxis, WC_MB_NumPMT_W, WC_MB_PMT_Spacing, 0.);
	...
	G4cout << "Coverage was calculated to be: "
		   << (WC_MB_NumPMT_W * WC_MB_NumPMT_D * WCPMT_crossarea / (WC_MB_Fid_Width * WC_MB_Fid_Depth)) << "\n"; // ***replica--modified original
	...
	G4VPhysicalVolume *phys_WC_Active_WxD_blacksheet_minus = new G4PVPlacement(0,						  // no rotation
	...
	G4VPhysicalVolume *phys_WC_Active_WxD_blacksheet_plus = new G4PVPlacement(0,						// no rotation
																			  position_blacksheet_WxD,	// its position
																			  logic_WC_BlacksheetWxD,	// its logical volume
																			  "WD_Blacksheet",			// its name
																			  logic_WC_Active_WxD_plus, // its mother volume
	...
	new G4LogicalBorderSurface("WaterBS_LW_PolySurface", phys_WC_Active_WxD_minus, phys_WC_Active_WxD_blacksheet_minus,
							   WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	new G4LogicalBorderSurface("WaterBS_LW_PolySurface", phys_WC_Active_WxD_plus, phys_WC_Active_WxD_blacksheet_plus,
							   WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	...
	const G4Transform3D rotation_WxD_minus = G4RotateY3D(CLHEP::pi / 2.);					  // minus rotates +90degrees around Y axis
	const G4Transform3D rotation_WxD_plus = G4RotateY3D(-CLHEP::pi / 2.);					  // plus rotates -90degrees around Y axis
	...
	G4cout << "-x axis offset for translation" << -((WC_MB_Fid_Length + WC_ActiveLayer_Depth) / 2.) / CLHEP::m << "CLHEP::m"
		   << "\n";
	...
	G4LogicalVolume *logic_WC_Active_LxD_minus = new G4LogicalVolume(WC_Active_LxD,
																	 WCSimMaterialsBuilder::Instance()->GetMaterial(water), "LxDActive_minus", 0, 0, 0);
	G4LogicalVolume *logic_WC_Active_LxD_plus = new G4LogicalVolume(WC_Active_LxD,
																	WCSimMaterialsBuilder::Instance()->GetMaterial(water), "LxDActive_plus", 0, 0, 0);
	G4Box *WC_BlacksheetLxD = new G4Box("WC_BlackSheetLxD", WC_MB_Fid_Length / 2, WC_MB_Fid_Depth / 2,
										WCBlackSheetThickness / 2.); //blacksheet material
	...
	G4SubtractionSolid *WC_BlacksheetLxD_donut = new G4SubtractionSolid("WC_BlacksheetLxD_donut", WC_BlacksheetLxD,
																		WC_BlacksheetLxD_donuthole, 0, G4ThreeVector(0., 0., 0.)); //this is now the blacksheet that will suround the PMT volumes  ***replica
	//***replica

	G4LogicalVolume *logic_WC_BlacksheetLxD = new G4LogicalVolume(WC_BlacksheetLxD_donut,
																  WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "LagicBlacksheet_LxD", 0, 0, 0); //***replica
	...
	G4Box *LxD_PMT_Row = new G4Box("LxD_PMT_Row", WC_MB_NumPMT_L * WC_MB_PMT_Spacing / 2, WC_MB_PMT_Spacing / 2,
								   WC_ActiveLayer_Depth / 2.);
	G4LogicalVolume *logic_LxD_PMT_Row = new G4LogicalVolume(LxD_PMT_Row,
															 WCSimMaterialsBuilder::Instance()->GetMaterial(water), "logic_LxD_PMT_Row", 0, 0, 0);
	G4Box *LxD_PMT_Volume = new G4Box("LxD_PMT_Volume", WC_MB_NumPMT_L * WC_MB_PMT_Spacing / 2,
									  WC_MB_NumPMT_D * WC_MB_PMT_Spacing / 2, WC_ActiveLayer_Depth / 2); //this is entire PMT volume
	G4LogicalVolume *logic_LxD_PMT_Volume = new G4LogicalVolume(LxD_PMT_Volume,
																WCSimMaterialsBuilder::Instance()->GetMaterial(water), "logic_LxD_PMT_Volume", 0, 0, 0);

	G4VPhysicalVolume *physiWC_LxD_PMT_Row = new G4PVReplica("WC_PMT_Row", logic_WC_PMT_Cell, logic_LxD_PMT_Row, kXAxis,
															 WC_MB_NumPMT_L, WC_MB_PMT_Spacing, 0.);

	G4VPhysicalVolume *physiWC_LxD_PMT_Volume = new G4PVReplica("WC_PMT_Volume", logic_LxD_PMT_Row,
																logic_LxD_PMT_Volume, kYAxis, WC_MB_NumPMT_D, WC_MB_PMT_Spacing, 0.);
	...
	G4cout << "Coverage was calculated to be: "
		   << (WC_MB_NumPMT_L * WC_MB_NumPMT_D * WCPMT_crossarea / (WC_MB_Fid_Length * WC_MB_Fid_Depth)) << "\n";
	...
	G4VPhysicalVolume *phys_WC_Active_LxD_blacksheet_minus = new G4PVPlacement(0,						  // no rotation
	...
	G4VPhysicalVolume *phys_WC_Active_LxD_blacksheet_plus = new G4PVPlacement(0,						// no rotation
																			  position_blacksheet_LxD,	// its position
																			  logic_WC_BlacksheetLxD,	// its logical volume
																			  "LD_Blacksheet",			// its name
																			  logic_WC_Active_LxD_plus, // its mother volume
	...
	new G4LogicalBorderSurface("WaterBS_LW_PolySurface", phys_WC_Active_LxD_minus, phys_WC_Active_LxD_blacksheet_minus,
							   WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	new G4LogicalBorderSurface("WaterBS_LW_PolySurface", phys_WC_Active_LxD_plus, phys_WC_Active_LxD_blacksheet_plus,
							   WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	...
	const G4Transform3D rotation_LxD_minus = G4RotateX3D(-CLHEP::pi / 2.);
	const G4Transform3D rotation_LxD_plus = G4RotateX3D(+CLHEP::pi / 2.);
	...
	new G4PVPlacement(0, G4ThreeVector(), logic_WC_MB_Fiducial, "physiMB_Fiducial", logic_WC_MB_tank_H20, false, 0,
					  false); // this should include now the fiducial volume and the pmt volumes around it.
...
G4LogicalVolume *WCSimDetectorConstruction::ConstructWC()
{
	...
	/*
	 ...
	 //  innerAnnulusRadius = WCIDRadius - WCPMTExposeHeight-1.*CLHEP::mm;
	 innerAnnulusRadius = WCIDRadius - fPMTConfigs[0].GetExposeHeight()-1.*CLHEP::mm;
	 outerAnnulusRadius = WCIDRadius + WCBlackSheetThickness + 1.*CLHEP::mm;//+ Stealstructure etc.
	 ...
	 WCRadius    = (WCIDDiameter/2. + WCBlackSheetThickness + 1.5*CLHEP::m)/cos(dPhi/2.) ; // TODO: OD
	 ...
	 0.0*CLHEP::m,
	 WCRadius+2.*CLHEP::m,
	 .5*WCLength+4.2*CLHEP::m,	//jl145 - per blueprint
	 0.*CLHEP::deg,
	 360.*CLHEP::deg);
	 ...
	 0.0*CLHEP::m,
	 WCRadius+1.*CLHEP::m, // add a bit of extra space
	 ...
	 for(int i = 0; i < 2 ; i++){
	 extraTowerRmin[i] = mainAnnulusRmin[i] != 0 ? mainAnnulusRmin[i]/cos(dPhi/2.)*cos((2.*CLHEP::pi-totalAngle)/2.) : 0.;
	 extraTowerRmax[i] = mainAnnulusRmax[i] != 0 ? mainAnnulusRmax[i]/cos(dPhi/2.)*cos((2.*CLHEP::pi-totalAngle)/2.) : 0.;
	 }
	 G4Polyhedra* solidWCExtraTower = new G4Polyhedra("WCextraTower",
	 totalAngle-2.*CLHEP::pi,//+dPhi/2., // phi start
	 2.*CLHEP::pi -  totalAngle // total angle.
	 -G4GeometryTolerance::GetInstance()->GetSurfaceTolerance()/(10.*CLHEP::m),
	 ...
	 totalAngle-2.*CLHEP::pi,//+dPhi/2., // phi start
	 2.*CLHEP::pi -  totalAngle -G4GeometryTolerance::GetInstance()->GetSurfaceTolerance()/(10.*CLHEP::m), //phi end
	 1, //NPhi-gon
	 2,
	 RingZ,
	 extraTowerRmin,
	 ...
	 for(int i = 0; i < 2; i++){
	 towerBSRmin[i] = annulusBlackSheetRmin[i]/cos(dPhi/2.)*cos((2.*CLHEP::pi-totalAngle)/2.);
	 towerBSRmax[i] = annulusBlackSheetRmax[i]/cos(dPhi/2.)*cos((2.*CLHEP::pi-totalAngle)/2.);
	 }
	 ...
	 G4double WCTyvekThickness = 1.0*CLHEP::mm; //completely made up
	 ...
	 G4double WCTVPMTSpacing = (WCSimTuningParams->GetTVSpacing())*CLHEP::cm;
	 ...
	 G4ThreeVector(	xoffset, yoffset, -0.5*CLHEP::m);
	 ...
	 G4RotationMatrix* WCPMTRotation = new G4RotationMatrix;
	 WCPMTRotation->rotateY(90.*CLHEP::deg);
	 WCPMTRotation->rotateX((2*CLHEP::pi-totalAngle)/2.);//align the PMT with the Cell

	 G4double towerWidth = WCIDRadius*tan(2*CLHEP::pi-totalAngle);
	 ...
	 for(G4double j = 0; j < WCPMTperCellVertical; j++){
	 G4ThreeVector PMTPosition =  G4ThreeVector(WCIDRadius/cos(dPhi/2.)*cos((2.*CLHEP::pi-totalAngle)/2.),
	 towerWidth/2.-(i+0.5)*horizontalSpacing,
	 -barrelCellHeight/2.+(j+0.5)*verticalSpacing);
	 PMTPosition.rotateZ(-(2*CLHEP::pi-totalAngle)/2.); // align with the symmetry
	 //axes of the cell
...
G4LogicalVolume *WCSimDetectorConstruction::ConstructCaps(G4int zflip)
{
	...
	capAssemblyHeight = (WCIDHeight - mainAnnulusHeight) / 2 + 1 * CLHEP::mm + WCBlackSheetThickness;
	std::cout << "capAssemblyHeight = " << capAssemblyHeight << "  WCIDHeight = " << WCIDHeight
			  << " mainAnnulusHeight = " << mainAnnulusHeight << std::endl;
	assert(0);

	G4Tubs *solidCapAssembly = new G4Tubs("CapAssembly", 0.0 * CLHEP::m, outerAnnulusRadius / cos(dPhi / 2.),
										  capAssemblyHeight / 2, 0. * CLHEP::deg, 360. * CLHEP::deg);

	G4LogicalVolume *logicCapAssembly = new G4LogicalVolume(solidCapAssembly,
															WCSimMaterialsBuilder::Instance()->GetMaterial(water), "CapAssembly", 0, 0, 0);

	//----------------------------------------------------
	// extra rings for the top and bottom of the annulus
	//---------------------------------------------------
	G4double borderAnnulusZ[3] = {-barrelCellHeight / 2. * zflip, (-barrelCellHeight / 2. + (WCIDRadius - innerAnnulusRadius)) * zflip, barrelCellHeight / 2. * zflip};
	...
	G4Polyhedra *solidWCBarrelBorderRing = new G4Polyhedra("WCBarrelBorderRing", 0. * CLHEP::deg, // phi start
														   totalAngle, (G4int)WCBarrelRingNPhi,	  //NPhi-gon
														   3, borderAnnulusZ, borderAnnulusRmin, borderAnnulusRmax);
	G4LogicalVolume *logicWCBarrelBorderRing = new G4LogicalVolume(solidWCBarrelBorderRing,
																   WCSimMaterialsBuilder::Instance()->GetMaterial(water), "WCBarrelRing", 0, 0, 0);
	//G4cout << *solidWCBarrelBorderRing << G4endl;

	G4VPhysicalVolume *physiWCBarrelBorderRing = new G4PVPlacement(0,
																   G4ThreeVector(0., 0., (capAssemblyHeight / 2. - barrelCellHeight / 2.) * zflip), logicWCBarrelBorderRing,
																   "WCBarrelBorderRing", logicCapAssembly, false, 0, true);
	...
	// --------------------------------------------------
	G4Polyhedra *solidWCBarrelBorderCell = new G4Polyhedra("WCBarrelBorderCell", -dPhi / 2., // phi start
														   dPhi,							 //total angle
														   1,								 //NPhi-gon
														   3, borderAnnulusZ, borderAnnulusRmin, borderAnnulusRmax);

	G4LogicalVolume *logicWCBarrelBorderCell = new G4LogicalVolume(solidWCBarrelBorderCell,
																   WCSimMaterialsBuilder::Instance()->GetMaterial(water), "WCBarrelBorderCell", 0, 0, 0);
	//G4cout << *solidWCBarrelBorderCell << G4endl;
	G4VPhysicalVolume *physiWCBarrelBorderCell = new G4PVReplica("WCBarrelBorderCell", logicWCBarrelBorderCell,
																 logicWCBarrelBorderRing, kPhi, (G4int)WCBarrelRingNPhi, dPhi, 0.);
	if (!debugMode)
	...
	G4VPhysicalVolume *physiWCBarrelBorderCellBlackSheet = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.),
																			 logicWCBarrelCellBlackSheet, "WCBarrelCellBlackSheet", logicWCBarrelBorderCell, false, 0, true);

	G4LogicalBorderSurface *WaterBSBarrelBorderCellSurface = new G4LogicalBorderSurface("WaterBSBarrelCellSurface",
																						physiWCBarrelBorderCell, physiWCBarrelBorderCellBlackSheet,
																						WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	...
	if (!(WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal))
	{
		...
		for (int i = 0; i < 3; i++)
		{
			extraBorderRmin[i] = borderAnnulusRmin[i] / cos(dPhi / 2.) * cos((2. * CLHEP::pi - totalAngle) / 2.);
			extraBorderRmax[i] = borderAnnulusRmax[i] / cos(dPhi / 2.) * cos((2. * CLHEP::pi - totalAngle) / 2.);
		}
		G4Polyhedra *solidWCExtraBorderCell = new G4Polyhedra("WCspecialBarrelBorderCell", totalAngle - 2. * CLHEP::pi,													  //+dPhi/2., // phi start
															  2. * CLHEP::pi - totalAngle - G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * CLHEP::m), //total phi
															  1,																										  //NPhi-gon
															  3, borderAnnulusZ, extraBorderRmin, extraBorderRmax);

		logicWCExtraBorderCell = new G4LogicalVolume(solidWCExtraBorderCell,
													 WCSimMaterialsBuilder::Instance()->GetMaterial(water), "WCspecialBarrelBorderCell", 0, 0, 0);
		//G4cout << *solidWCExtraBorderCell << G4endl;

		G4VPhysicalVolume *physiWCExtraBorderCell = new G4PVPlacement(0,
																	  G4ThreeVector(0., 0., (capAssemblyHeight / 2. - barrelCellHeight / 2.) * zflip), logicWCExtraBorderCell,
																	  "WCExtraTowerBorderCell", logicCapAssembly, false, 0, true);

		logicWCExtraBorderCell->SetVisAttributes(G4VisAttributes::Invisible);

		G4VPhysicalVolume *physiWCExtraBorderBlackSheet = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.),
																			logicWCTowerBlackSheet, "WCExtraTowerBlackSheet", logicWCExtraBorderCell, false, 0, true);

		G4LogicalBorderSurface *WaterBSExtraBorderCellSurface = new G4LogicalBorderSurface("WaterBSBarrelCellSurface",
	...
	G4double capZ[4] = {(-WCBlackSheetThickness - 1. * CLHEP::mm) * zflip, WCBarrelPMTOffset * zflip, WCBarrelPMTOffset * zflip, (WCBarrelPMTOffset + (WCIDRadius - innerAnnulusRadius)) * zflip};
	G4double capRmin[4] = {0., 0., 0., 0.};
	G4double capRmax[4] = {outerAnnulusRadius, outerAnnulusRadius, WCIDRadius, innerAnnulusRadius};
	G4VSolid *solidWCCap;
	if (WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)
	{
		solidWCCap = new G4Polyhedra("WCCap", 0. * CLHEP::deg, // phi start
									 totalAngle,			   //phi end
	...
	else
	{
		...
		G4Polyhedra *mainPart = new G4Polyhedra("WCCapMainPart", 0. * CLHEP::deg, // phi start
												totalAngle,						  //phi end
												(int)WCBarrelRingNPhi,			  //NPhi-gon
												4,								  // 2 z-planes
												capZ,							  //position of the Z planes
		...
		for (int i = 0; i < 4; i++)
		{
			extraCapRmin[i] = capRmin[i] != 0. ? capRmin[i] / cos(dPhi / 2.) * cos((2. * CLHEP::pi - totalAngle) / 2.) : 0.;
			extraCapRmax[i] = capRmax[i] != 0. ? capRmax[i] / cos(dPhi / 2.) * cos((2. * CLHEP::pi - totalAngle) / 2.) : 0.;
		}
		G4Polyhedra *extraSlice = new G4Polyhedra("WCCapExtraSlice", totalAngle - 2. * CLHEP::pi,															  // phi start
												  2. * CLHEP::pi - totalAngle - G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * CLHEP::m), //total phi
		...
		solidWCCap = new G4UnionSolid("WCCap", mainPart, extraSlice);

		//G4cout << *solidWCCap << G4endl;
	}
	// G4cout << *solidWCCap << G4endl;
	G4LogicalVolume *logicWCCap = new G4LogicalVolume(solidWCCap, WCSimMaterialsBuilder::Instance()->GetMaterial(water),
													  "WCCapPolygon", 0, 0, 0);

	G4VPhysicalVolume *physiWCCap = new G4PVPlacement(0,																								// no rotation
													  G4ThreeVector(0., 0., (-capAssemblyHeight / 2. + 1 * CLHEP::mm + WCBlackSheetThickness) * zflip), // its position
	...
	G4double capBlackSheetRmax[4] = {WCIDRadius + WCBlackSheetThickness, WCIDRadius + WCBlackSheetThickness, WCIDRadius + WCBlackSheetThickness, WCIDRadius + WCBlackSheetThickness};
	G4VSolid *solidWCCapBlackSheet;
	if (WCBarrelRingNPhi * WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)
	{
		solidWCCapBlackSheet = new G4Polyhedra("WCCapBlackSheet", 0. * CLHEP::deg, // phi start
	...
	else
	{
		...
		G4Polyhedra *mainPart = new G4Polyhedra("WCCapBlackSheetMainPart", 0. * CLHEP::deg, // phi start
												totalAngle,									//phi end
												WCBarrelRingNPhi,							//NPhi-gon
												4,											//  z-planes
												capBlackSheetZ,								//position of the Z planes
		...
		{
			extraBSRmin[i] =
				capBlackSheetRmin[i] != 0. ? capBlackSheetRmin[i] / cos(dPhi / 2.) * cos((2. * CLHEP::pi - totalAngle) / 2.) : 0.;
			extraBSRmax[i] =
				capBlackSheetRmax[i] != 0. ? capBlackSheetRmax[i] / cos(dPhi / 2.) * cos((2. * CLHEP::pi - totalAngle) / 2.) : 0.;
		}
		G4Polyhedra *extraSlice = new G4Polyhedra("WCCapBlackSheetextraSlice", totalAngle - 2. * CLHEP::pi,													  // phi start
												  2. * CLHEP::pi - totalAngle - G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / (10. * CLHEP::m), //
												  WCBarrelRingNPhi,																							  //NPhi-gon
												  4,																										  //  z-planes
												  capBlackSheetZ,																							  //position of the Z planes
		...
		solidWCCapBlackSheet = new G4UnionSolid("WCCapBlackSheet", mainPart, extraSlice);
	}
	G4LogicalVolume *logicWCCapBlackSheet = new G4LogicalVolume(solidWCCapBlackSheet,
																WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "WCCapBlackSheet", 0, 0, 0);
	G4VPhysicalVolume *physiWCCapBlackSheet = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), logicWCCapBlackSheet,
																"WCCapBlackSheet", logicWCCap, false, 0, true);
	G4LogicalBorderSurface *WaterBSBottomCapSurface = new G4LogicalBorderSurface("WaterBSCapPolySurface", physiWCCap,
																				 physiWCCapBlackSheet, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	...
	for (int i = -CapNCell; i < CapNCell; i++)
	{
		for (int j = -CapNCell; j < CapNCell; j++)
		{
			...
			if (((sqrt(xoffset * xoffset + yoffset * yoffset) + fPMTConfigs[0].GetRadius()) < WCCapEdgeLimit))
			{
				...
				G4VPhysicalVolume *physiCapPMT = new G4PVPlacement(WCCapPMTRotation, cellpos, // its position
																   logicWCPMT,				  // its logical volume
																   "WCPMT",					  // its name
	...
	G4cout << "Coverage was calculated to be: "
		   << (icopy * fPMTConfigs[0].GetRadius() * fPMTConfigs[0].GetRadius() / (WCIDRadius * WCIDRadius)) << "\n";
	...
	for (G4double i = 0; i < WCPMTperCellHorizontal; i++)
	{
		for (G4double j = 0; j < WCPMTperCellVertical; j++)
		{
			G4ThreeVector PMTPosition = G4ThreeVector(WCIDRadius, -barrelCellWidth / 2. + (i + 0.5) * horizontalSpacing,
													  (-barrelCellHeight / 2. + (j + 0.5) * verticalSpacing) * zflip);

			G4VPhysicalVolume *physiWCBarrelBorderPMT = new G4PVPlacement(WCPMTRotation,							  // its rotation
																		  PMTPosition, logicWCPMT,					  // its logical volume
																		  "WCPMT",									  // its name
																		  logicWCBarrelBorderCell,					  // its mother volume
																		  false,									  // no boolean operations
																		  (int)(i * WCPMTperCellVertical + j), true); // no particular field
		...
		for (G4double i = 0; i < (WCBarrelNumPMTHorizontal - WCBarrelRingNPhi * WCPMTperCellHorizontal); i++)
		{
			for (G4double j = 0; j < WCPMTperCellVertical; j++)
			{
				G4ThreeVector PMTPosition = G4ThreeVector(
					WCIDRadius / cos(dPhi / 2.) * cos((2. * CLHEP::pi - totalAngle) / 2.),
					towerWidth / 2. - (i + 0.5) * horizontalSpacing,
					(-barrelCellHeight / 2. + (j + 0.5) * verticalSpacing) * zflip);
				PMTPosition.rotateZ(-(2 * CLHEP::pi - totalAngle) / 2.); // align with the symmetry
																		 //axes of the cell
