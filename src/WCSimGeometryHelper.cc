...
void WCSimGeometryHelper::Run()
{
    ...
    fDoc.clear();
		...
		const char * nameStr = fDoc.allocate_string((GetName()).c_str());
		rapidxml::xml_attribute<> *nameAttr = fDoc.allocate_attribute("name", nameStr);
		...
		if( std::string(coverageType) == WCSimGeometryEnums::PhotodetectorLimit_t::AsString(WCSimGeometryEnums::PhotodetectorLimit_t::kPercentCoverage))
		{
...
{
	std::string region;
	if(isTop){ region = WCSimGeometryEnums::DetectorRegion_t::AsString(WCSimGeometryEnums::DetectorRegion_t::kTop); }
	else { region = WCSimGeometryEnums::DetectorRegion_t::AsString(WCSimGeometryEnums::DetectorRegion_t::kBottom); }
...
void WCSimGeometryHelper::SetWallZones(rapidxml::xml_node<> * parentNode)
{
  ...
  while( std::find(fSetWallZones.begin(), fSetWallZones.end(), 0 ) != fSetWallZones.end() )
  {
    ...
    {
      if( fSetWallZones.at(i) == 0 ) { std::cout << i << " "; }
    }
    std::cout << std::endl;
    ...
    {
      if( jZone == zonesToSet.size() - 1 && jZone > 0 ) { std::cout << "and "; }
...
bool WCSimGeometryHelper::ValidateBoundaries(std::vector<double> boundaries)
{
	...
	if(!isGood){ std::cout << "A problem was found with your zone angle definitions" << std::endl;}
	return isGood;
}
...
std::vector<std::pair<double, double> > WCSimGeometryHelper::GetZoneBoundaries()
{
	...
	else
	{
		...
		if( AskOptionString(options) == "degrees" ){ conversionFactor = M_PI / 180.0; }
		...
		std::vector<std::string> &uniquePMTs)
{
  ...
  std::cout << "Now let's specify the unit cell for this zone:" << std::endl;
    ...
    rapidxml::xml_node<> * pmtNode = fDoc.allocate_node(rapidxml::node_element,"PMT");
		...
		{
			uniquePMTs.push_back(std::string(name));
		}
...
std::string WCSimGeometryHelper::AskOptionString(
		std::vector<std::string> options)
{
	std::ostringstream optionText;
