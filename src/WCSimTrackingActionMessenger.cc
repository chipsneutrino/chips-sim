...
WCSimTrackingActionMessenger::WCSimTrackingActionMessenger(
		WCSimTrackingAction* WCSimTA) :
		fTrackingAction(WCSimTA)
{
...
void WCSimTrackingActionMessenger::SetNewValue(G4UIcommand* command,
		G4String newValue)
{
	 if( command == PercentCherenkovPhotonsToDraw )
	 {
      double newPercent = PercentCherenkovPhotonsToDraw->GetNewDoubleValue(newValue);
		  ...
		  {
			  std::cerr << "You've asked to save < 1% of Cherenkov photons.  Are you sure you specified a percentage and not a decimal?" << std::endl;
			  std::cerr << "I'm going to assume you meant what you asked for and save " << newPercent << "% of photons" << std::endl;
		  }
