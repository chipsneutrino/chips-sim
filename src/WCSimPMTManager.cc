...
void WCSimPMTManager::FillPMTAttribute(WCSimPMTConfig &pmt, const std::string &attrName, const std::string &attrValue)
{
	...
	else if (attrName == "radius")
	{
		double tempVal;
		ss >> tempVal;
		pmt.SetRadius(tempVal * CLHEP::m);
	}
	else if (attrName == "exposeHeight")
	{
		double tempVal;
		ss >> tempVal;
		pmt.SetExposeHeight(tempVal * CLHEP::m);
	}
	else if (attrName == "unsensitiveHeight")
	{
		double tempVal;
		ss >> tempVal;
		pmt.SetUnsensitiveHeight(tempVal * CLHEP::m);
	}
	else if (attrName == "glassThickness")
	{
		double tempVal;
		ss >> tempVal;
		pmt.SetGlassThickness(tempVal * CLHEP::m);
	}
...
WCSimPMTConfig WCSimPMTManager::GetPMTByName(std::string name)
{
	...
	{
		std::cerr << "WCSIMPMTManager::GetPMTByName: PMT " << name
				  << " does not exist. Returned default WCSimPMTConfig object." << std::endl;
		std::cout << "Available PMTs are: " << std::endl;
		for (unsigned int i = 0; i < fPMTVector.size(); ++i)
		{
