...
WCSimEmissionProfileMaker::WCSimEmissionProfileMaker(G4String filename) :
		fRho(0x0), fGFine(0x0), fGCoarse(0x0) {
...
void WCSimEmissionProfileMaker::NormaliseG() {
	...
	for (int iRow = 1; iRow <= fSCosThetaCoarse->GetYaxis()->GetNbins(); ++iRow) {
		...
		for (int iCol = 1; iCol <= fSCosThetaCoarse->GetXaxis()->GetNbins(); ++iCol) {
			if (fSCosThetaCoarse->GetBinContent(iCol, iRow) > 0
					&& fSCosThetaCoarse->GetBinContent(iCol, iRow) < minEntries) {
		...
		for (int iCol = 1; iCol <= fSCosThetaFine->GetXaxis()->GetNbins(); ++iCol) {
			if (fSCosThetaFine->GetBinContent(iCol, iRow) > 0
					&& fSCosThetaFine->GetBinContent(iCol, iRow) < minEntries) {
		...
		for (int jCol = 1; jCol <= fSCosThetaCoarse->GetXaxis()->GetNbins(); ++jCol) {
			if (fSCosThetaCoarse->GetBinContent(jCol, iRow) > 0) {
				fGCoarse->SetBinContent(jCol, iRow,
						(fSCosThetaCoarse->GetBinContent(jCol, iRow) / fSCosThetaCoarse->GetXaxis()->GetBinWidth(jCol))
								/ (sumTheta * sumWidth));
			}
		}
		for (int jCol = 1; jCol <= fSCosThetaFine->GetXaxis()->GetNbins(); ++jCol) {
			if (fSCosThetaFine->GetBinContent(jCol, iRow) > 0) {
				...
				fGFine->SetBinContent(jCol, iRow,
						fSCosThetaFine->GetBinContent(jCol, iRow) / fSCosThetaFine->GetXaxis()->GetBinWidth(jCol)
								/ (sumTheta * sumWidth));
			}
		}
	}
...
void WCSimEmissionProfileMaker::MakeHistograms() {
	...
	fSCosThetaCheckBinning = new TH2F("fSCosThetaCheckBinning", ";cos#theta;s (cm);photons", nSecBins, cosThetaBins, 40,
			fSMin, fSMax);
}
