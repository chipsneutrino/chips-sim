...
// WCSimCherenkovBuilder Constructor
WCSimCherenkovBuilder::WCSimCherenkovBuilder(G4int DetConfig) : WCSimDetectorConstruction(DetConfig), fConstructed(false), fGeoConfig(NULL)
{

	fBlacksheetThickness = 5 * CLHEP::mm;
	fWhitesheetThickness = fBlacksheetThickness;
	fPMTHolderLength = 154.8 * CLHEP::mm;
	//	fPMTHolderLength = 15.48 * CLHEP::mm;
	fPlanePipeRadius = 19.05 * CLHEP::mm; // 1.5 Inch Pipes
	fPlanePipeStep = 1850 * CLHEP::mm;	  // Approximate POM Length
	...
	fBarrelRadius = -999 * CLHEP::m; // Barrel is a cylinder - this is its actual radius
	fBarrelHeight = -999 * CLHEP::m;
	fBarrelLengthForCells = -999 * CLHEP::m;

	fVetoRadiusInside = -999 * CLHEP::m; // Radius is to centre of wall not the vertex
	fVetoRadiusOutside = -999 * CLHEP::m;
	fVetoHeight = -999 * CLHEP::m;

	fPrismRadiusInside = -999 * CLHEP::m; // Radius is to centre of wall not the vertex
	fPrismRadiusOutside = -999 * CLHEP::m;
	fPrismHeight = -999 * CLHEP::m;

	fPrismRingRadiusInside = -999 * CLHEP::m; // Radius is to centre of wall not the vertex
	fPrismRingRadiusOutside = -999 * CLHEP::m;

	fPrismRingSegmentRadiusInside = -999 * CLHEP::m; // To centre of segment not edge
	fPrismRingSegmentRadiusInside = -999 * CLHEP::m;
	fPrismRingSegmentDPhi = -999 * CLHEP::m;

	fPrismRingSegmentBSRadiusInside = -999 * CLHEP::m; // To centre not edge
	fPrismRingSegmentBSRadiusOutside = -999 * CLHEP::m;

	fCapAssemblyHeight = -999 * CLHEP::m;
	fCapAssemblyRadius = -999 * CLHEP::m;

	fCapRingHeight = -999 * CLHEP::m;
	fCapRingRadiusInside = -999 * CLHEP::m;
	fCapRingRadiusOutside = -999 * CLHEP::m;

	fCapRingSegmentDPhi = -999 * CLHEP::m;
	fCapRingSegmentHeight = -999 * CLHEP::m;
	fCapRingSegmentRadiusInside = -999 * CLHEP::m;
	fCapRingSegmentRadiusOutside = -999 * CLHEP::m;

	fCapRingSegmentBSHeight = -999 * CLHEP::m;
	fCapRingSegmentBSRadiusInside = -999 * CLHEP::m;
	fCapRingSegmentBSRadiusOutside = -999 * CLHEP::m;

	fCapPolygonRadius = -999 * CLHEP::m;
	fCapPolygonHeight = -999 * CLHEP::m;

	fCapPolygonCentreRadius = -999 * CLHEP::m;
	fCapPolygonCentreHeight = -999 * CLHEP::m;

	fCapPolygonEdgeBSRadiusInside = -999 * CLHEP::m;
	fCapPolygonEdgeBSRadiusOutside = -999 * CLHEP::m;
	fCapPolygonEdgeBSHeight = -999 * CLHEP::m;

	fCapPolygonEndBSRadius = -999 * CLHEP::m;
	fCapPolygonEndBSHeight = -999 * CLHEP::m;
}

// WCSimCherenkovBuilder Destructor
WCSimCherenkovBuilder::~WCSimCherenkovBuilder()
{
	...
	{
		if (fPrismWallLogics.at(iWall) != NULL)
		{
			delete fPrismWallLogics.at(iWall);
		}
		if (fPrismWallPhysics.at(iWall) != NULL)
		{
			delete fPrismWallPhysics.at(iWall);
		}
		if (fPrismRingLogics.at(iWall) != NULL)
		{
			delete fPrismRingLogics.at(iWall);
		}
	...
	{
		if (fSegmentLogics.at(iSegment) != NULL)
		{
			delete fSegmentLogics.at(iSegment);
		}
	...
	if (fCapLogicBottom != NULL)
	{
		delete fCapLogicBottom;
	}
	if (fCapLogicBottomRing != NULL)
	{
		delete fCapLogicBottomRing;
	}
	if (fCapLogicTop != NULL)
	{
		delete fCapLogicTop;
	}
	if (fPrismLogic != NULL)
...
void WCSimCherenkovBuilder::ConstructEnvironment()
{
	...
	// The water barrel is placed in a cylinfder tub of water, representing the lake
	...
	G4double lakeOR = innerRadius + shoreDistance * CLHEP::m; // outer radius of lake
	G4double lakeHeight = innerHeight + 20. * CLHEP::m;
	...
	G4Tubs *lakeTubs = new G4Tubs("lake", lakeIR, lakeOR, lakeHeight, 0.0 * CLHEP::deg, 360.0 * CLHEP::deg);
...
void WCSimCherenkovBuilder::ConstructFrame()
{
	...
	G4Tubs *barrelTubs = new G4Tubs("barrelTubs", 0, fBarrelRadius, fBarrelHeight / 2.0, 0. * CLHEP::deg, 360. * CLHEP::deg);
	...
	prismRotation->rotateZ(180.0 / (double)fGeoConfig->GetNSides() * CLHEP::deg);
	fBarrelPhysics = new G4PVPlacement(prismRotation, G4ThreeVector(0., 0., 0.), fBarrelLogic, "barrelTubs", fLakeLogic,
									   false, 0);
}
...
void WCSimCherenkovBuilder::ConstructVeto()
{
	...
	// The radii are measured to the centre of the surfaces
	// (tangent distance). Thus distances between the corner and the centre are bigger.
	...
	G4Polyhedra *vetoSolid = new G4Polyhedra("vetoPrism", 0. * CLHEP::deg,		// phi start
											 360.0 * CLHEP::deg,				// phi end
	...
	G4VPhysicalVolume *vBarrel = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), fVetoLogic, "vetoPrism", fBarrelLogic,
												   false, 0, true);

	//  G4double topCapZ[2] = {0.5*fVetoHeight+0.0001*CLHEP::mm,0.5*fVetoHeight+fWhitesheetThickness};
	...
	G4Polyhedra *vetoTopSolid = new G4Polyhedra("vetoTopCap", 0. * CLHEP::deg, 360. * CLHEP::deg, (G4int)fGeoConfig->GetNSides(), 2,
												topCapZ, topCapRMin, topCapRMax);
	...
	//  G4double bottomCapZ[2] = {-(0.5*fVetoHeight+0.0001*CLHEP::mm),-(0.5*fVetoHeight+fWhitesheetThickness)};
	...
	G4Polyhedra *vetoBottomSolid = new G4Polyhedra("vetoBottomCap", 0. * CLHEP::deg, 360. * CLHEP::deg,
												   (G4int)fGeoConfig->GetNSides(), 2, bottomCapZ, bottomCapRMin, bottomCapRMax);
	fVetoBottomLogic = new G4LogicalVolume(vetoBottomSolid,
										   WCSimMaterialsBuilder::Instance()->GetMaterial("Whitesheet"), "vetoBottomCap", 0, 0, 0);
	fVetoBottomLogic->SetVisAttributes(vetoColour);
	G4VPhysicalVolume *vBottom = new G4PVPlacement(0,
												   G4ThreeVector(0., 0., -0.5 * (fVetoHeight + fWhitesheetThickness)), fVetoBottomLogic, "vetoBottomCap",
												   fBarrelLogic, false, 0, true);

	G4LogicalBorderSurface *WaterWSVetoBarrelSurface = NULL;
	WaterWSVetoBarrelSurface = new G4LogicalBorderSurface("WaterWSVetoBarrelSurface", fBarrelPhysics, vBarrel,
														  WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterWSCellSurface"));

	G4LogicalBorderSurface *WaterWSVetoTopSurface = NULL;
	WaterWSVetoTopSurface = new G4LogicalBorderSurface("WaterWSVetoTopSurface", fBarrelPhysics, vTop,
													   WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterWSCellSurface"));

	G4LogicalBorderSurface *WaterWSVetoBottomSurface = NULL;
	WaterWSVetoBottomSurface = new G4LogicalBorderSurface("WaterWSVetoBottomSurface", fBarrelPhysics, vBottom,
														  WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterWSCellSurface"));
	return;
}
...
void WCSimCherenkovBuilder::CreatePrism()
{
	...
	std::cout << "INNER VOLUME = " << fPrismRadiusInside << ", " << fPrismRadiusOutside << ", " << fPrismHeight
			  << std::endl;

	// Now make the volumes
	G4Polyhedra *prismSolid = new G4Polyhedra("prism", 0. * CLHEP::deg,			 // phi start
											  360.0 * CLHEP::deg,				 // phi end
											  (G4int)fGeoConfig->GetNSides(),	 //number of sides
											  2, mainAnnulusZ,					 // number and location of z planes
											  mainAnnulusRmin, mainAnnulusRmax); // inner and outer radii

	fPrismLogic = new G4LogicalVolume(prismSolid, WCSimMaterialsBuilder::Instance()->GetMaterial("Water"), "prism", 0,
									  0, 0);
	...
	new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), fPrismLogic, "prism", fBarrelLogic, false, 0, true);
}
...
void WCSimCherenkovBuilder::CreatePrismWalls()
{
	...
	for (unsigned int iZone = 0; iZone < fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kWall);
		 ++iZone)
	{
		...
		prismWallRotation->rotateZ(
			360.0 * (fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kWall) - iZone - 0.5) / (double)fGeoConfig->GetNSides() * CLHEP::deg);

		// Now make the volumes
		std::stringstream ss;
		ss << "prism" << iZone;
		G4String elName = ss.str();
		G4Polyhedra *prismWallSolid = new G4Polyhedra(elName, -0.5 * 360.0 / ((double)fGeoConfig->GetNSides()) * CLHEP::deg, // phi start
													  360.0 / ((double)fGeoConfig->GetNSides()) * CLHEP::deg,				 // phi width
													  1,																	 //number of sides
													  2, prismWallZ,														 // number and location of z planes
													  prismWallRmin, prismWallRmax);										 // inner and outer radii
		if (fPrismWallLogics.at(iZone) != NULL)
		{
			delete fPrismWallLogics.at(iZone);
		}
		fPrismWallLogics.at(iZone) = new G4LogicalVolume(prismWallSolid,
														 WCSimMaterialsBuilder::Instance()->GetMaterial("Water"), ss.str().c_str(), 0, 0, 0);
		fPrismWallLogics.at(iZone)->SetVisAttributes(prismWallColor);
		if (fPrismWallPhysics.at(iZone) != NULL)
		{
			delete fPrismWallPhysics.at(iZone);
		}
		fPrismWallPhysics.at(iZone) = new G4PVPlacement(prismWallRotation, G4ThreeVector(0, 0, 0),
														fPrismWallLogics.at(iZone), ss.str().c_str(), fPrismLogic, false, 0);
	}
}

void WCSimCherenkovBuilder::CreatePrismRings()
{
	// Slice the prism up length-ways
		 ...
		 ++iZone)
	{
		...
		std::cout << "GeoConfig thinks there are "
				  << fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kWall) << " zones " << std::endl;
		std::cout << "Current zone = " << iZone << std::endl;
		std::cout << "fPrismRingHeight = " << fPrismRingHeight.at(iZone) << "  fPrismRingSegmentRadiusInside = "
				  << fPrismRingSegmentRadiusInside << " and fPrismRingSegmentRadiusOutside = " << fPrismRingRadiusOutside
				  << std::endl;
		...
		G4Polyhedra *prismRingSolid = new G4Polyhedra(name, -fPrismRingSegmentDPhi / 2.0, // phi start
		...
		if (fPrismRingLogics.at(iZone) != NULL)
		{
			delete fPrismRingLogics.at(iZone);
		}
		fPrismRingLogics.at(iZone) = new G4LogicalVolume(prismRingSolid, pureWater, name, 0, 0, 0);

		if (fPrismRingPhysics.at(iZone) != NULL)
		{
			delete fPrismRingPhysics.at(iZone);
		}
		...
		fPrismRingPhysics.at(iZone) = new G4PVReplica("prismRing", fPrismRingLogics.at(iZone),
													  fPrismWallLogics.at(iZone), kZAxis, (G4int)fWallCellsZ.at(iZone), fPrismRingHeight.at(iZone));
...
void WCSimCherenkovBuilder::CreateRingSegments()
{
	...
	{
		// Zip these up into arrays to pass to Geant4
		...
		G4Polyhedra *segmentSolid = new G4Polyhedra(name, -fPrismRingSegmentDPhi / 2.0, // phi start
													fPrismRingSegmentDPhi,				//total Phi
													1,									//NPhi-gon
													2, RingZ, mainAnnulusRmin, mainAnnulusRmax);

		fSegmentLogics.push_back(new G4LogicalVolume(segmentSolid, pureWater, name, 0, 0, 0));

		G4RotationMatrix *WCSegmentRotation = new G4RotationMatrix;
		WCSegmentRotation->rotateY((0.5 * fGeoConfig->GetNSides() / 360.0) * CLHEP::deg);

		G4VPhysicalVolume *segmentPhysic = new G4PVPlacement(WCSegmentRotation, G4ThreeVector(0., 0., 0.),
															 fSegmentLogics.at(iSide), name, fPrismRingLogics.at(iSide), false, iSide);
	...
	for (unsigned int iSide = 0; iSide < fGeoConfig->GetNSides(); ++iSide)
	{
		G4double segmentWhitesheetZ[2] = {-fPrismRingSegmentBSHeight.at(iSide) / 2.0, fPrismRingSegmentBSHeight.at(
																						  iSide) /
																						  2.0};
		G4double segmentWhitesheetRmax[2] = {fPrismRingSegmentWSRadiusOutside, fPrismRingSegmentWSRadiusOutside};
		G4double segmentWhitesheetRmin[2] = {fPrismRingSegmentWSRadiusInside, fPrismRingSegmentWSRadiusInside};

		std::stringstream ss;
		ss << "segmentBlacksheet" << iSide;
		const char *name = ss.str().c_str();
		G4Polyhedra *segmentWhitesheetSolid = new G4Polyhedra(name, -0.5 * fPrismRingSegmentDPhi, // phi start
															  fPrismRingSegmentDPhi,			  //total phi
															  1,								  //NPhi-gon
															  2, segmentWhitesheetZ, segmentWhitesheetRmin, segmentWhitesheetRmax);

		G4LogicalVolume *segmentWhitesheetLogic = new G4LogicalVolume(segmentWhitesheetSolid,
																	  WCSimMaterialsBuilder::Instance()->GetMaterial("Whitesheet"), name, 0, 0, 0);

		G4RotationMatrix *WCSegmentWSRotation = new G4RotationMatrix;
		WCSegmentWSRotation->rotateY(0. * CLHEP::deg);

		G4VPhysicalVolume *segmentWhitesheetPhysic = new G4PVPlacement(WCSegmentWSRotation, G4ThreeVector(0., 0., 0.),
																	   segmentWhitesheetLogic, name, fSegmentLogics.at(iSide), false, iSide);

		G4LogicalBorderSurface *WaterWSBarrelCellSurface = NULL;
		WaterWSBarrelCellSurface = new G4LogicalBorderSurface("WaterWSBarrelCellSurface", fSegmentPhysics.at(iSide),
															  segmentWhitesheetPhysic, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterWSCellSurface"));

		G4VisAttributes *WCBarrelWhitesheetCellVisAtt = new G4VisAttributes(G4Colour(0.3, 0.3, 0.3));
		if (fDebugMode)
	...
	for (unsigned int iSide = 0; iSide < fGeoConfig->GetNSides(); ++iSide)
	{
		G4double segmentBlacksheetZ[2] = {-fPrismRingSegmentBSHeight.at(iSide) / 2.0, fPrismRingSegmentBSHeight.at(
																						  iSide) /
																						  2.0};
		G4double segmentBlacksheetRmax[2] = {fPrismRingSegmentBSRadiusOutside, fPrismRingSegmentBSRadiusOutside};
		G4double segmentBlacksheetRmin[2] = {fPrismRingSegmentBSRadiusInside, fPrismRingSegmentBSRadiusInside};

		std::stringstream ss;
		ss << "segmentBlacksheet" << iSide;
		const char *name = ss.str().c_str();
		G4Polyhedra *segmentBlacksheetSolid = new G4Polyhedra(name, -0.5 * fPrismRingSegmentDPhi, // phi start
															  fPrismRingSegmentDPhi,			  //total phi
															  1,								  //NPhi-gon
															  2, segmentBlacksheetZ, segmentBlacksheetRmin, segmentBlacksheetRmax);

		G4LogicalVolume *segmentBlacksheetLogic = new G4LogicalVolume(segmentBlacksheetSolid,
																	  WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), name, 0, 0, 0);

		G4RotationMatrix *WCSegmentBSRotation = new G4RotationMatrix;
		WCSegmentBSRotation->rotateY(0. * CLHEP::deg);

		G4VPhysicalVolume *segmentBlacksheetPhysic = new G4PVPlacement(WCSegmentBSRotation, G4ThreeVector(0., 0., 0.),
																	   segmentBlacksheetLogic, name, fSegmentLogics.at(iSide), false, iSide);

		G4LogicalBorderSurface *WaterBSBarrelCellSurface = NULL;
		WaterBSBarrelCellSurface = new G4LogicalBorderSurface("WaterBSBarrelCellSurface", fSegmentPhysics.at(iSide),
															  segmentBlacksheetPhysic, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));

		G4VisAttributes *WCBarrelBlacksheetCellVisAtt = new G4VisAttributes(G4Colour(0.2, 0.9, 0.2));
		if (fDebugMode)
			segmentBlacksheetLogic->SetVisAttributes(WCBarrelBlacksheetCellVisAtt);
		else
			segmentBlacksheetLogic->SetVisAttributes(G4VisAttributes::Invisible);
	}
}
...
void WCSimCherenkovBuilder::PlaceBarrelPMTs()
{
		 ...
		 ++iZone)
	{
		...
		std::vector<Double_t> pmtFaceTheta = fGeoConfig->GetCellPMTFaceTheta(
			WCSimGeometryEnums::DetectorRegion_t::kWall, iZone);
		std::vector<Double_t> pmtFacePhi = fGeoConfig->GetCellPMTFacePhi(WCSimGeometryEnums::DetectorRegion_t::kWall,
																		 iZone);
		std::vector<WCSimGeometryEnums::PMTDirection_t> pmtFaceType = fGeoConfig->GetCellPMTFaceType(
			WCSimGeometryEnums::DetectorRegion_t::kWall, iZone);
		...
		for (G4double i = 0; i < fWallCellsX.at(iZone); i++)
		{
			...
			G4ThreeVector offsetToUnitCell = G4ThreeVector(0, 0.5 * (widthPerCell - fWallCellSize.at(iZone)),
														   0.5 * (heightPerCell - fWallCellSize.at(iZone))); // Offset to the bottom left corner of the unit cell itself
			...
			for (unsigned int nPMT = 0; nPMT < unitCell->GetNumPMTs(); ++nPMT)
			{
				...
				std::cout << std::endl
						  << "Placing PMT " << nPMT << "in wall cell " << i << std::endl;
				std::cout << "PMT position in cell = " << pmtCellPosition.x() / CLHEP::m << "," << pmtCellPosition.y() / CLHEP::m
						  << "in m" << std::endl;
				...
				std::cout << "Cell mother height and width: " << heightPerCell / 2. << "  " << segmentWidth / 2.
						  << std::endl;
				WCSimPMTConfig config = unitCell->GetPMTPlacement(nPMT).GetPMTConfig();
				std::cout << " PMT logical volume name = " << fPMTBuilder.GetPMTLogicalVolume(config)->GetName()
						  << std::endl;
				...
				if (pmtFaceType.at(nPMT) == WCSimGeometryEnums::PMTDirection_t::kArbitrary)
					...
					*tmpWCPMTRotation *= GetBarrelPMTFaceRotation(pmtFaceType.at(nPMT), iZone);

				//Stefano : Plane Pipes
				double pomD =
					(fGeoConfig->UsePMTSupport(WCSimGeometryEnums::PMTSupport_t::kPOM)) ? 2 * fPlanePipeRadius + fPMTHolderLength + 5 * CLHEP::mm : 0;
				...
				double dTheta = fabs(tmpWCPMTRotation->thetaY() / CLHEP::deg - 90);
				double dR = radius * sin(dTheta * CLHEP::deg);
		...
		if (fGeoConfig->UsePMTSupport(WCSimGeometryEnums::PMTSupport_t::kPOM))
			PlaceBarrelPlanePipes(iZone);
...
void WCSimCherenkovBuilder::GetMeasurements()
{
	...
	double epsilon = 0.00001 * CLHEP::mm; // Add a tiny bit of gap between supposedly overlapping volumes
										  // to prevent Geant4 getting stuck at the tracking action
	int nEpsilons = 6;

	// Barrel measurements
	fBarrelRadius = fGeoConfig->GetOuterRadius() + 1 * CLHEP::m;														  // Make it 1m bigger in radius
	fBarrelHeight = fGeoConfig->GetInnerHeight() + 2 * fBlacksheetThickness + 2 * GetMaxCapExposeHeight() + 2 * CLHEP::m; // and half-height
	...
	fPrismRadiusOutside = fGeoConfig->GetInnerRadius() + fBlacksheetThickness + fWhitesheetThickness + 2 * GetMaxBarrelExposeHeight() + (2 * fPlanePipeRadius + fPMTHolderLength + 5 * CLHEP::mm) + nEpsilons * epsilon;
	...
	assert(--nEpsilons > 0);
	//    fPrismRingSegmentRadiusInside = fPrismRadiusInside-1*CLHEP::mm + epsilon;
	//    fPrismRingSegmentRadiusOutside = fPrismRadiusOutside+1*CLHEP::mm - epsilon;
	fPrismRingSegmentRadiusInside = fPrismRadiusInside + 1 * CLHEP::mm + epsilon;
	fPrismRingSegmentRadiusOutside = fPrismRadiusOutside - 1 * CLHEP::mm - epsilon;
	fPrismRingSegmentDPhi = 360 * CLHEP::deg / fGeoConfig->GetNSides();

	assert(--nEpsilons > 0);
	fPrismRingSegmentBSRadiusInside = fPrismRingSegmentRadiusInside + GetMaxBarrelExposeHeight() + (2 * fPlanePipeRadius + fPMTHolderLength + 5 * CLHEP::mm); // make room for pmt/LC  and  pipes
	...
	std::cout << "-Radii: " << fCapAssemblyRadius << ", " << fCapRingRadiusInside << ", " << fCapRingRadiusOutside
			  << std::endl;
	...
	std::cout << "-Radii: " << std::setprecision(15) << fPrismRingSegmentRadiusInside << ", " << std::setprecision(15)
			  << fPrismRingSegmentRadiusOutside << ", " << std::setprecision(15) << fPrismRingSegmentBSRadiusInside
			  << ", " << std::setprecision(15) << fPrismRingSegmentBSRadiusOutside << ", " << std::setprecision(15)
			  << fPrismRingSegmentWSRadiusInside << ", " << std::setprecision(15) << fPrismRingSegmentWSRadiusOutside
			  << std::endl;

	std::cout << "-Radii: " << std::setprecision(15) << fCapRingSegmentRadiusInside << ", " << std::setprecision(15)
			  << fCapRingSegmentRadiusOutside << ", " << std::setprecision(15) << fCapRingSegmentBSRadiusInside << ", "
			  << std::setprecision(15) << fCapRingSegmentBSRadiusOutside << ", " << std::setprecision(15)
			  << fCapRingSegmentWSRadiusInside << ", " << std::setprecision(15) << fCapRingSegmentWSRadiusOutside
			  << std::endl;
...
double WCSimCherenkovBuilder::GetMaxCapExposeHeight()
{
	...
	for (unsigned int iZone = 0; iZone < fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kBottom);
		 ++iZone)
	{
		WCSimUnitCell *cell2 = GetBottomUnitCell(iZone);
		double expHeight2 = cell2->GetCellExposeHeight();
		if (expHeight2 > maxHeight)
		{
			maxHeight = expHeight2;
		}
	}

	//Stefano: make room for pipes and PMT holder
	if (fGeoConfig->UsePMTSupport(WCSimGeometryEnums::PMTSupport_t::kPOM))
		maxHeight += 2 * fPlanePipeRadius + fPMTHolderLength;
...
void WCSimCherenkovBuilder::ConstructUnitCells()
{
		 ...
		 ++iZone)
	{
		...
		std::vector<std::string> pmtNames = fGeoConfig->GetCellPMTName(WCSimGeometryEnums::DetectorRegion_t::kWall,
																	   iZone);
		...
		{
			std::cout << "PMT = " << pmtNames.at(i) << "  X = " << pmtX.at(i) << " / m = " << pmtX.at(i) / CLHEP::m
					  << std::endl;
	...
	for (unsigned int iZone = 0; iZone < fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kTop); ++iZone)
	{
		...
		std::vector<std::string> pmtNames = fGeoConfig->GetCellPMTName(WCSimGeometryEnums::DetectorRegion_t::kTop,
																	   iZone);
		assert(pmtX.size() == pmtY.size() && pmtX.size() == pmtNames.size());
		 ...
		 ++iZone)
	{
		...
		std::vector<std::string> pmtNames = fGeoConfig->GetCellPMTName(WCSimGeometryEnums::DetectorRegion_t::kBottom,
																	   iZone);
		assert(pmtX.size() == pmtY.size() && pmtX.size() == pmtNames.size());
...
WCSimUnitCell *WCSimCherenkovBuilder::GetUnitCell(WCSimGeometryEnums::DetectorRegion_t region, int zone)
{
	WCSimUnitCell *cell = NULL;
	if (region == WCSimGeometryEnums::DetectorRegion_t::kTop)
	{
		cell = GetTopUnitCell(zone);
	}
	if (region == WCSimGeometryEnums::DetectorRegion_t::kBottom)
	{
		cell = GetBottomUnitCell(zone);
	}
	if (region == WCSimGeometryEnums::DetectorRegion_t::kWall)
	{
		cell = GetWallUnitCell(zone);
	}
	assert(cell != NULL);
	return cell;
}
...
double WCSimCherenkovBuilder::GetOptimalEndcapCellSize(WCSimGeometryEnums::DetectorRegion_t region, int zoneNum)
{
	...
	if (fGeoConfig->GetLimitPMTNumber())
	{
		...
		double maxCoverage = pmtArea * maxNumCells / WCSimPolygonTools::GetSliceAreaFromAngles(fGeoConfig->GetNSides(), fGeoConfig->GetOuterRadius(), fGeoConfig->GetZoneThetaStart(region, zoneNum), fGeoConfig->GetZoneThetaEnd(region, zoneNum));

		std::cout << "Maximum coverage with available PMTs for region " << region.AsString() << ", zone " << zoneNum
				  << " = " << maxCoverage << std::endl;
		std::cout << "Uses " << maxNumCells << " cells, where each cell is "
				  << endcapCell.GetCellSizeForCoverage(maxCoverage) << " in size" << std::endl;
	...
	else
	{
		if (fGeoConfig->GetUseOverallCoverage() || fGeoConfig->GetUseZonalCoverage())
		{
			...
			maxNumCells = (int)(targetCoverage * WCSimPolygonTools::GetSliceAreaFromAngles(fGeoConfig->GetNSides(), fGeoConfig->GetOuterRadius(), fGeoConfig->GetZoneThetaStart(region, zoneNum), fGeoConfig->GetZoneThetaEnd(region, zoneNum)) / endcapCell.GetPhotocathodeArea());
			std::cout << "Finding coverage for region " << region.AsString() << ", zone " << zoneNum << " = "
					  << targetCoverage << std::endl;
			std::cout << "Target coverage = " << targetCoverage << " with a cell containing " << endcapCell.GetNumPMTs()
					  << " PMTs" << std::endl;
			std::cout << "Cell photocathode area = " << endcapCell.GetPhotocathodeArea() << " so we need ";
			std::cout << maxNumCells << " to cover a zone area of "
					  << WCSimPolygonTools::GetSliceAreaFromAngles(fGeoConfig->GetNSides(), fGeoConfig->GetOuterRadius(),
	...
	double capPolygonOuterRadius = WCSimPolygonTools::GetOuterRadiusFromInner(fGeoConfig->GetNSides(),
																			  fCapRingRadiusInside - 0.0001 * CLHEP::mm);
	...
	for (int iIter = 0; iIter < 100; ++iIter)
	{
		...
		while (xPos < squareSide)
		{
			...
			while (fabs(yPos) < squareSide)
			{
				...
				for (unsigned int iPMT = 0; iPMT < pmtNames.size(); ++iPMT)
				{
					...
					G4TwoVector pmtTopLeft(cellSide * pmtX.at(iPMT) / CLHEP::m - pmtRad.at(iPMT),
										   cellSide * pmtY.at(iPMT) / CLHEP::m + pmtRad.at(iPMT));
					G4TwoVector pmtTopRight(cellSide * pmtX.at(iPMT) / CLHEP::m + pmtRad.at(iPMT),
											cellSide * pmtY.at(iPMT) / CLHEP::m + pmtRad.at(iPMT));
					G4TwoVector pmtBottomLeft(cellSide * pmtX.at(iPMT) / CLHEP::m - pmtRad.at(iPMT),
											  cellSide * pmtY.at(iPMT) / CLHEP::m - pmtRad.at(iPMT));
					G4TwoVector pmtBottomRight(cellSide * pmtX.at(iPMT) / CLHEP::m + pmtRad.at(iPMT),
											   cellSide * pmtY.at(iPMT) / CLHEP::m - pmtRad.at(iPMT));
		...
		{
			double pmtArea = endcapCell.GetPhotocathodeArea();
			double coverage = pmtArea * cellsInPolygon / WCSimPolygonTools::GetSliceAreaFromAngles(fGeoConfig->GetNSides(), fGeoConfig->GetOuterRadius(), fGeoConfig->GetZoneThetaStart(region, zoneNum), fGeoConfig->GetZoneThetaEnd(region, zoneNum));
	...
	if (fGeoConfig->GetLimitPMTNumber())
	{
		std::cout << "bestNumCells = " << bestNumCells << " (maxNumCells = " << maxNumCells
				  << ") - chosen on iteration " << bestIter << " side = " << bestSide << std::endl
				  << std::endl;
	}
	else
	{
		std::cout << "bestCoverage = " << bestCoverage << " (targetCoverage = " << targetCoverage
				  << ") - chosen on iteration " << bestIter << " side = " << bestSide << std::endl
				  << std::endl;
	}
...
double WCSimCherenkovBuilder::GetOptimalWallCellSize(int iZone)
{
	...
	if (fGeoConfig->GetLimitPMTNumber())
	{
		...
		std::cout << "Maximum coverage with available PMTs for region "
				  << " kWall, zone " << iZone << " = "
				  << maxCoverage << std::endl;
	...
	if (!canHaveMorePMTs)
	{
		...
		double minSize = wallCell->GetCellSizeForCoverage(
			wallCell->GetPhotocathodeArea() * maxNumCells / (wallHeight * wallSide));
		...
		if (wallHeight / bestY < bestLength)
		{
			bestLength = wallHeight / bestY;
		}
	...
	else
	{
		...
		//		std::cout << "Wall height = " << wallHeight << " ( * CLHEP::m = " << wallHeight * CLHEP::m << ")" << std::endl;
		...
		// We need an CLHEP::m x n arrangement of cells that gives coverage close
...
void WCSimCherenkovBuilder::ConstructEndCapFrame(G4int zflip)
{
	...
	//	G4Tubs* capSolid = new G4Tubs("CapAssembly", 0.0*CLHEP::m,
	G4Polyhedra *capSolid = new G4Polyhedra("CapAssembly", 0.0,		 // phi start
											360 * CLHEP::deg,		 //total Phi
											fGeoConfig->GetNSides(), //NPhi-gon
											2, capAssemblyZ, capAssemblyRMin, capAssemblyRMax);

	capLogic = new G4LogicalVolume(capSolid, pureWater, "CapAssembly", 0, 0, 0);

	if (zflip == -1)
	{
		fCapLogicTop = capLogic;
		std::cout << "TOP " << fCapLogicTop->GetName() << std::endl;
	}
	else
	{
		fCapLogicBottom = capLogic;
		std::cout << "BOTTOM " << fCapLogicBottom->GetName() << std::endl;
	}
	return;
}
...
void WCSimCherenkovBuilder::ConstructEndCapRings(G4int zflip)
{
	...
	G4Polyhedra *solidEndCapRing = new G4Polyhedra("EndCapRing", 0. * CLHEP::deg,				// phi start
												   360.0 * CLHEP::deg, fGeoConfig->GetNSides(), //NPhi-gon
												   2, borderRingZ, borderRingRMin, borderRingRMax);
	G4LogicalVolume *logicEndCapRing = new G4LogicalVolume(solidEndCapRing, pureWater, "WCBarrelRing", 0, 0, 0);

	logicEndCapRing->SetVisAttributes(WCCapRingVisAtt);

	if (zflip == -1)
	{
		fCapLogicTopRing = logicEndCapRing;
	}
	else
	{
		fCapLogicBottomRing = logicEndCapRing;
	}
	...
	new G4PVPlacement(endCapRotation, G4ThreeVector(0., 0., (0.5 * fCapAssemblyHeight - 0.5 * fCapRingHeight) * zflip),
					  logicEndCapRing, "EndCapRing", capLogic, false, 0, true);
	std::cout << "endCapRing placed in capLogic at z = " << (0.5 * fCapAssemblyHeight - 0.5 * fCapRingHeight) * zflip
			  << " with height " << fCapRingHeight << std::endl;
}
...
void WCSimCherenkovBuilder::ConstructEndCapRingSegments(G4int zflip)
{
	...
	// Now divide each ring (an n-gon prism) into n rectangular segments
	G4Polyhedra *capSegmentSolid = new G4Polyhedra("EndCapSegment", -fCapRingSegmentDPhi / 2.0, // phi start
												   fCapRingSegmentDPhi,							//total Phi
												   1,											//NPhi-gon
												   2, capRingZ, capRingRMin, capRingRMax);

	G4LogicalVolume *capSegmentLogic = new G4LogicalVolume(capSegmentSolid, pureWater, "EndCapSegment", 0, 0, 0);
	capSegmentLogic->SetVisAttributes(WCCapRingSegmentVisAtt);

	G4LogicalVolume *capRingLogic = NULL;
	if (zflip == -1)
	{
		capRingLogic = fCapLogicTopRing;
	}
	else
	{
		capRingLogic = fCapLogicBottomRing;
	}
	assert(capRingLogic != NULL);

	G4VPhysicalVolume *capSegmentPhysic = new G4PVReplica("EndCapSegment", capSegmentLogic, capRingLogic, kPhi,
														  fGeoConfig->GetNSides(), fCapRingSegmentDPhi, 0.);

	std::cout << "EndCapSegment placed " << std::setprecision(15) << capRingRMin[0] << ", " << std::setprecision(15)
			  << capRingRMax[0] << std::endl;
	...
	G4Polyhedra *capSegmentBlacksheetSolid = new G4Polyhedra("EndCapSegmentBlacksheet", -0.5 * fCapRingSegmentDPhi, // phi start
															 fCapRingSegmentDPhi,									//total phi
															 1,														//NPhi-gon
															 2, capSegmentBlacksheetZ, capSegmentBlacksheetRmin, capSegmentBlacksheetRmax);

	G4LogicalVolume *capSegmentBlacksheetLogic = new G4LogicalVolume(capSegmentBlacksheetSolid,
																	 WCSimMaterialsBuilder::Instance()->GetMaterial("Blacksheet"), "EndCapSegmentBlacksheet", 0, 0, 0);
	...
	G4VPhysicalVolume *capSegmentBlacksheetPhysic = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.),
																	  capSegmentBlacksheetLogic, "EndCapSegmentBlacksheet", capSegmentLogic, false, 0, true);
	...
	WaterBSCapCellSurface = new G4LogicalBorderSurface("WaterBSCapCellSurface", capSegmentPhysic,
													   capSegmentBlacksheetPhysic, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));
	...
	G4Polyhedra *capSegmentWhitesheetSolid = new G4Polyhedra("EndCapSegmentWhitesheet", -0.5 * fCapRingSegmentDPhi, // phi start
															 fCapRingSegmentDPhi,									//total phi
															 1,														//NPhi-gon
	...
	G4LogicalVolume *capSegmentWhitesheetLogic = new G4LogicalVolume(capSegmentWhitesheetSolid,
																	 WCSimMaterialsBuilder::Instance()->GetMaterial("Whitesheet"), "EndCapSegmentWhitesheet", 0, 0, 0);

	G4VPhysicalVolume *capSegmentWhitesheetPhysic = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.),
																	  capSegmentWhitesheetLogic, "EndCapSegmentWhitesheet", capSegmentLogic, false, 0, true);
	...
	WaterBSCapCellSurface = new G4LogicalBorderSurface("WaterWSCapCellSurface", capSegmentPhysic,
													   capSegmentWhitesheetPhysic, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterWSCellSurface"));
...
void WCSimCherenkovBuilder::ConstructEndCapSurfaces(G4int zflip)
{
	...
	G4VSolid *solidWCCapEndBlacksheet = new G4Polyhedra("WCCapEndBlacksheet", 0. * CLHEP::deg, // phi start
														360.0 * CLHEP::deg,					   //total phi
														fGeoConfig->GetNSides(),			   //NPhi-gon
														2,									   //  z-planes
	...
	G4LogicalVolume *logicWCCapEndBlacksheet = new G4LogicalVolume(solidWCCapEndBlacksheet, blacksheet,
																   "WCCapEndBlacksheet", 0, 0, 0);
	capBSPhysics = new G4PVPlacement(0, G4ThreeVector(0., 0., (0.5 * fCapPolygonEndBSHeight + 0.00001 * CLHEP::mm) * zflip),
									 logicWCCapEndBlacksheet, "WCCapEndBlacksheet", capLogic, false, 0, true);
	...
	G4VSolid *solidWCCapEndWhitesheet = new G4Polyhedra("WCCapEndWhitesheet", 0. * CLHEP::deg, // phi start
														360.0 * CLHEP::deg,					   //total phi
														fGeoConfig->GetNSides(),			   //NPhi-gon
														2, capEndWSZ, capEndWSRmin, capEndWSRmax);

	G4LogicalVolume *logicWCCapEndWhitesheet = new G4LogicalVolume(solidWCCapEndWhitesheet, whitesheet,
																   "WCCapEndWhitesheet", 0, 0, 0);
	capWSPhysics = new G4PVPlacement(0, G4ThreeVector(0., 0., (-0.5 * fCapPolygonEndWSHeight - 0.00001 * CLHEP::mm) * zflip),
									 logicWCCapEndWhitesheet, "WCCapEndWhitesheet", capLogic, false, 0, true);
...
void WCSimCherenkovBuilder::PlaceEndCapPMTs(G4int zflip)
{
	...
	if (zflip == -1)
	{
		WCCapPMTRotation->rotateY(180. * CLHEP::deg);
		capLogic = fCapLogicTop;
	...
	for (unsigned int iZone = 0; iZone < fGeoConfig->GetNumZones(region); ++iZone)
	{
		...
		while (xPos < squareSide)
		{
			...
			while (fabs(yPos) < squareSide)
			{
				...
				for (unsigned int iPMT = 0; iPMT < pmtNames.size(); ++iPMT)
				{
					...
					// std::cout << "PMT " << iPMT << " at " << pmtX.at(iPMT)/CLHEP::m << ", " << pmtY.at(iPMT)/CLHEP::m << " (r = " << pmtRad.at(iPMT) << ")" << std::endl;
					G4TwoVector pmtTopLeft(cellSide * (pmtX.at(iPMT) / CLHEP::m) - pmtRad.at(iPMT),
										   cellSide * (pmtY.at(iPMT) / CLHEP::m) + pmtRad.at(iPMT));
					...
					if (cellInPolygonSlice)
					{
						...
						if (pmtFaceType.at(iPMT) == WCSimGeometryEnums::PMTDirection_t::kArbitrary)
							*tmpWCCapPMTRotation *= GetArbitraryPMTFaceRotation(pmtFaceTheta.at(iPMT),
																				pmtFacePhi.at(iPMT));
						else
							*tmpWCCapPMTRotation *= GetEndcapPMTFaceRotation(pmtFaceType.at(iPMT), zflip);
						...
						{
							G4RotationMatrix rotation;
							rotation.rotateY(180. * CLHEP::deg);
						...
						if (fNumPMTs == 0 || 1)
						{
							...
							// std::cout << "PMT position in cell = " << pmtCellPosition.x() / CLHEP::m << "," << pmtCellPosition.y() / CLHEP::m << "in CLHEP::m" << std::endl;
						...
						if (pmtFaceType.at(iPMT) != WCSimGeometryEnums::PMTDirection_t::kVeto)
						{ // check PMT position for Plane Pipes only for non Veto PMT
		...
		/// Plane Pipes placement needs to be just after the corresponding PMT placement
		if (fGeoConfig->UsePMTSupport(WCSimGeometryEnums::PMTSupport_t::kPOM))
			PlaceEndCapPlanePipes(zflip, iZone);
	}
...
void WCSimCherenkovBuilder::PlaceEndCapPlanePipes(G4int zflip, G4int zone)
{
	...
	PlanePipeRotation->rotateX(90 * CLHEP::deg);

	G4RotationMatrix *PlanePipeShortRotation = new G4RotationMatrix;
	PlanePipeShortRotation->rotateX(90 * CLHEP::deg);
	PlanePipeShortRotation->rotateY(90 * CLHEP::deg);
	...
	for (int xit = 0; xit < fEndCapExtremePMTPos_Max.size(); ++xit)
	{
		...
		if (pipeLength <= 0)
			continue; // No pipe if min and max extreme position are identical
		...
		G4Tubs *planePipeTube = new G4Tubs(name, 0, fPlanePipeRadius, pipeLength / 2, 0 * CLHEP::deg, 360 * CLHEP::deg);

		fPlanePipeLogics.push_back(new G4LogicalVolume(planePipeTube, PVC, name));
		int iLogicPipe = fPlanePipeLogics.size() - 1;

		G4SurfaceProperty *OpWaterPlanePipeSurface =
			(G4SurfaceProperty *)(WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterPlanePipeSurface"));

		ss << "EndCapWaterPlanePipeSurface" << zone << "_" << xit;
		G4String sname = ss.str();
		G4LogicalSkinSurface *EcpWaterPlanePipeSurface = new G4LogicalSkinSurface(sname,
																				  fPlanePipeLogics.at(iLogicPipe), OpWaterPlanePipeSurface);

		fPlanePipePhysics.push_back(
			new G4PVPlacement(PlanePipeRotation,
							  (fEndCapExtremePMTPos_Max.at(xit) + fEndCapExtremePMTPos_Min.at(xit)) / 2,
							  fPlanePipeLogics.at(iLogicPipe), name, capLogic, false, 0));

		if (xit < fEndCapExtremePMTPos_Max.size() - 1)
		{

			double shortPipeLength = ((fEndCapExtremePMTPos_Max.at(xit + 1))[0] - (fEndCapExtremePMTPos_Min.at(xit))[0]) - 2 * fPlanePipeRadius - 0.1 * CLHEP::mm; // Distance between Parallel Pipes (alon X dir)

			if (shortPipeLength <= 0)
				continue;
			...
			G4Tubs *shortPlanePipeTube = new G4Tubs(name, 0, fPlanePipeRadius, shortPipeLength / 2, 0 * CLHEP::deg, 360 * CLHEP::deg);
			...
			sname = ss.str();
			G4LogicalSkinSurface *EcpWaterPlanePipeShortSurface = new G4LogicalSkinSurface(sname,
																						   fPlanePipeLogics.at(iLogicPipe), OpWaterPlanePipeSurface);
			...
			for (int sit = 0; sit < nPlanes; ++sit)
			{
				...
				G4ThreeVector shortShift(
					((fEndCapExtremePMTPos_Max.at(xit + 1))[0] - (fEndCapExtremePMTPos_Min.at(xit))[0]),
					shortPipeExcess / 2 + sit * fPlanePipeStep, 0);

				if ((fEndCapExtremePMTPos_Max.at(xit + 1))[1] < (fEndCapExtremePMTPos_Min.at(xit) + shortShift)[1])
					break;
				...
				fPlanePipePhysics.push_back(
					new G4PVPlacement(PlanePipeShortRotation,
									  (fEndCapExtremePMTPos_Min.at(xit) + shortShift + deltaShift),
									  fPlanePipeLogics.at(iLogicPipe), name, capLogic, false, nPipes));
...
void WCSimCherenkovBuilder::PlaceBarrelPlanePipes(G4int zone)
{
	...
	G4String sname = ss.str();
	G4LogicalSkinSurface *BrlWaterPlanePipeSurface = new G4LogicalSkinSurface(sname, fPlanePipeLogics.at(iLogicPipe),
	...
	G4RotationMatrix *PlanePipeShortRotation = new G4RotationMatrix;
	PlanePipeShortRotation->rotateZ(90 * CLHEP::deg);
	...
	for (int zit = 0; zit < fBarrelExtremePMTPos_Max.size(); ++zit)
	{
		...
		fPlanePipePhysics.push_back(
			new G4PVPlacement(PlanePipeRotation,
							  ...
							  fPlanePipeLogics.at(iLogicPipe), name, fSegmentLogics.at(zone), false, zit));

		double wallSide = 2.0 * fPrismRingSegmentBSRadiusInside * sin(M_PI / fGeoConfig->GetNSides()); // Side length of walls (copyed from above ... )
		double shortPipeLength = (wallSide / fWallCellsX.at(zone)) / 2 - fPlanePipeRadius - 0.1 * CLHEP::mm;
		...
		sname = ss.str();
		G4LogicalSkinSurface *BrlWaterPlanePipeShortSurface = new G4LogicalSkinSurface(sname,
																					   fPlanePipeLogics.at(iLogicPipe), OpWaterPlanePipeSurface);
		...
		for (int sit = 0; sit < nPlanes; ++sit)
		{
			...
			G4ThreeVector shortShift1(0, shortPipeExcess / 2 + sit * fPlanePipeStep,
									  shortPipeLength / 2 + fPlanePipeRadius + 0.1 * CLHEP::mm);
			G4ThreeVector shortShift2(0, shortPipeExcess / 2 + sit * fPlanePipeStep,
									  -shortPipeLength / 2 - fPlanePipeRadius - 0.1 * CLHEP::mm);
			...
			fPlanePipePhysics.push_back(
				new G4PVPlacement(PlanePipeShortRotation,
								  ...
								  fPlanePipeLogics.at(iLogicPipe), name, fSegmentLogics.at(zone), false, nPipes));
...
{
	// std::cout << "mainAnnulusHeight = " << mainAnnulusHeight << " (in CLHEP::m = " << mainAnnulusHeight/CLHEP::m << ")" << std::endl;
	// std::cout << "capAssemblyHeight = " << capAssemblyHeight << " (in CLHEP::m = " << capAssemblyHeight/CLHEP::m << ")" << std::endl;
	// std::cout << "InnerHeight=  " << fGeoConfig->GetInnerHeight() << " (in CLHEP::m = " << fGeoConfig->GetInnerHeight()/CLHEP::m << ")" << std::endl;
	// std::cout << "BarrelLengthForCells = " << GetBarrelLengthForCells() << " (in CLHEP::m = " << GetBarrelLengthForCells()/CLHEP::m << ")" << std::endl;
	// std::cout << "fBlacksheet thickness = " << fBlacksheetThickness << " (in CLHEP::m = " << fBlacksheetThickness/CLHEP::m << ")" << std::endl;
	...
	G4VPhysicalVolume *physiTopCapAssembly = new G4PVPlacement(0,
															   ...
															   G4ThreeVector(0., 0., 0.5 * (fBarrelLengthForCells + fCapAssemblyHeight)), fCapLogicTop, "TopCapAssembly",
															   fBarrelLogic, false, 0, true);
	G4VPhysicalVolume *physiBottomCapAssembly = new G4PVPlacement(0,
																  //  									G4ThreeVector(0.,0., -0.5 * (fGeoConfig->GetInnerHeight() + fCapAssemblyHeight)),
																  G4ThreeVector(0., 0., -0.5 * (fBarrelLengthForCells + fCapAssemblyHeight)), fCapLogicBottom,
																  "BottomCapAssembly", fBarrelLogic, false, 0, true);
	...
	WaterBSTopCapEndSurface = new G4LogicalBorderSurface("WaterBSTopCapPolySurface", physiTopCapAssembly,
														 fCapBSTopPhysics, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));

	G4LogicalBorderSurface *WaterWSTopCapEndSurface = NULL;
	WaterWSTopCapEndSurface = new G4LogicalBorderSurface("WaterWSTopCapPolySurface", physiTopCapAssembly,
														 fCapWSTopPhysics, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterWSCellSurface"));
	G4LogicalBorderSurface *WaterBSBottomCapEndSurface = NULL;
	WaterBSBottomCapEndSurface = new G4LogicalBorderSurface("WaterBSBottomCapPolySurface", physiBottomCapAssembly,
															fCapBSBottomPhysics, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterBSCellSurface"));

	G4LogicalBorderSurface *WaterWSBottomCapEndSurface = NULL;
	WaterWSBottomCapEndSurface = new G4LogicalBorderSurface("WaterWSBottomCapPolySurface", physiBottomCapAssembly,
															fCapWSBottomPhysics, WCSimMaterialsBuilder::Instance()->GetOpticalSurface("WaterWSCellSurface"));
	return;
}
...
void WCSimCherenkovBuilder::SetPositions()
{
	...
	totalAngle = 2.0 * CLHEP::pi * CLHEP::rad * (WCBarrelRingNPhi * WCPMTperCellHorizontal / WCBarrelNumPMTHorizontal);
	...
	//  innerAnnulusRadius = fGeoConfig->GetOuterRadius() - WCPMTExposeHeight-1.*CLHEP::mm;
	//==>	 innerAnnulusRadius = fGeoConfig->GetOuterRadius() - fPMTConfigs[0].GetExposeHeight()
	innerAnnulusRadius = fGeoConfig->GetOuterRadius() - fPMTConfigs[0].GetMaxExposeHeight() - 1. * CLHEP::mm;
	outerAnnulusRadius = fGeoConfig->GetOuterRadius() + fBlacksheetThickness + 1. * CLHEP::mm; //+ Stealstructure etc.
	...
	WCLength = WCIDHeight + 2 * 2.3 * CLHEP::m; //jl145 - reflects top veto blueprint, cf. Farshid Feyzi
	WCRadius = (WCIDDiameter / 2. + fBlacksheetThickness + 1.5 * CLHEP::m) / cos(dPhi / 2.);
	...
	G4cout << "Computed tolerance = " << G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / CLHEP::mm << " mm"
		   << G4endl;
...
void WCSimCherenkovBuilder::Update()
{
	...
	for (unsigned int iPipe = 0; iPipe < fPlanePipeLogics.size(); ++iPipe)
	{
		...
		if (fPlanePipeLogics.at(iPipe) != NULL)
		{
			delete fPlanePipeLogics.at(iPipe);
		}
	}
	...
	for (unsigned int iSegment = 0; iSegment < fSegmentLogics.size(); ++iSegment)
	{
		...
		if (fSegmentPhysics.at(iSegment) != NULL)
		{
			delete fSegmentPhysics.at(iSegment);
		}
	}
...
double WCSimCherenkovBuilder::GetZoneCoverage(WCSimGeometryEnums::DetectorRegion_t region, int zone)
{
	...
	else
	{
		...
		return fGeoConfig->GetZonalCoverageFraction(region, zone);
	} // Default to same as above for now - tries for max coverage with a fixed number of PMTs
	assert(coverage != 0.0);
	return coverage;
}
...
G4RotationMatrix WCSimCherenkovBuilder::GetArbitraryPMTFaceRotation(double theta, double phi)
{
	...
	G4RotationMatrix rotation;
	rotation.setTheta(theta * CLHEP::deg);
	rotation.setPhi(phi * CLHEP::deg);
...
G4RotationMatrix WCSimCherenkovBuilder::GetEndcapPMTFaceRotation(WCSimGeometryEnums::PMTDirection_t type, G4int zflip)
{
	...
	if (type == WCSimGeometryEnums::PMTDirection_t::kInwards)
		...
		;
	else if (type == WCSimGeometryEnums::PMTDirection_t::kAngledUpstream && zflip == 1)
		rotation.rotateY((45) * CLHEP::deg); //
	else if (type == WCSimGeometryEnums::PMTDirection_t::kAngledUpstream && zflip == -1)
		rotation.rotateY((-45) * CLHEP::deg); //
	else
		G4cerr << "PMT Rotation Option NOT Defined or Implemented" << G4endl;
...
G4RotationMatrix WCSimCherenkovBuilder::GetBarrelPMTFaceRotation(WCSimGeometryEnums::PMTDirection_t type, G4int zone)
{
	...
	else if (type == WCSimGeometryEnums::PMTDirection_t::kAngledUpstream)
	{
		...
		if (fabs(segmentRotation->phiX() / CLHEP::deg) < 90) // Rotate PMTs for backward facing segments only
