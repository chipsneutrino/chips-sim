...
void WCSimCherenkovBuilder::ConstructEnvironment() {
	...
	G4Tubs* lakeTubs = new G4Tubs("lake", lakeIR, lakeOR, lakeHeight, 0.0 * deg, 360.0 * deg);
...
//  std::cout << "VETO VOLUME = " << fVetoRadiusInside << ", " << fVetoRadiusOutside << ", " << fVetoHeight << std::endl;
	...
	G4Polyhedra* vetoSolid = new G4Polyhedra("vetoPrism", 0. * deg, // phi start
	360.0 * deg, // phi end
...
//  G4double topCapZ[2] = {0.5*fVetoHeight+0.0001*mm,0.5*fVetoHeight+fWhitesheetThickness};
	...
	G4Polyhedra *vetoTopSolid = new G4Polyhedra("vetoTopCap", 0. * deg, 360. * deg, (G4int) fGeoConfig->GetNSides(), 2,
...
void WCSimCherenkovBuilder::CreatePrism() {
	...
	// Now make the volumes
	G4Polyhedra* prismSolid = new G4Polyhedra("prism", 0. * deg,  // phi start
	360.0 * deg, // phi end
	(G4int) fGeoConfig->GetNSides(), //number of sides
...
void WCSimCherenkovBuilder::CreatePrismWalls() {
	...
	for (unsigned int iZone = 0; iZone < fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kWall);
		...
		prismWallRotation->rotateZ(
				360.0 * (fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kWall) - iZone - 0.5)
						/ (double) fGeoConfig->GetNSides() * deg);

		// Now make the volumes
		std::stringstream ss;
		ss << "prism" << iZone;
		G4String elName = ss.str();
		G4Polyhedra* prismWallSolid = new G4Polyhedra(elName, -0.5 * 360.0 / ((double) fGeoConfig->GetNSides()) * deg, // phi start
	...
	for (unsigned int iZone = 0; iZone < fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kWall);
		...
		for (G4double i = 0; i < fWallCellsX.at(iZone); i++) {
			...
			for (unsigned int nPMT = 0; nPMT < unitCell->GetNumPMTs(); ++nPMT) {
				...
				std::cout << "PMT position in cell = " << pmtCellPosition.x() / m << "," << pmtCellPosition.y() / m
						<< "in m" << std::endl;
				...
				//Stefano : Plane Pipes
				double pomD =
						(fGeoConfig->UsePMTSupport(WCSimGeometryEnums::PMTSupport_t::kPOM)) ?
								2 * fPlanePipeRadius + fPMTHolderLength + 5 * mm : 0;
...
void WCSimCherenkovBuilder::GetMeasurements() {
	...
	fPrismRadiusOutside = fGeoConfig->GetInnerRadius() + fBlacksheetThickness + fWhitesheetThickness
			+ 2 * GetMaxBarrelExposeHeight() + (2 * fPlanePipeRadius + fPMTHolderLength + 5 * mm) + nEpsilons * epsilon;
...
//    fPrismRingSegmentRadiusOutside = fPrismRadiusOutside+1*mm - epsilon;
	...
	fPrismRingSegmentBSRadiusInside = fPrismRingSegmentRadiusInside + GetMaxBarrelExposeHeight()
			+ (2 * fPlanePipeRadius + fPMTHolderLength + 5 * mm);  // make room for pmt/LC  and  pipes
...
void WCSimCherenkovBuilder::ConstructUnitCells() {
	...
	for (unsigned int iZone = 0; iZone < fGeoConfig->GetNumZones(WCSimGeometryEnums::DetectorRegion_t::kWall);
		...
		for (unsigned int i = 0; i < pmtNames.size(); ++i) {
			std::cout << "PMT = " << pmtNames.at(i) << "  X = " << pmtX.at(i) << " / m = " << pmtX.at(i) / m
					<< std::endl;
	...
	if (fGeoConfig->GetLimitPMTNumber()) {
		...
		double maxCoverage = pmtArea * maxNumCells
				/ WCSimPolygonTools::GetSliceAreaFromAngles(fGeoConfig->GetNSides(), fGeoConfig->GetOuterRadius(),
						fGeoConfig->GetZoneThetaStart(region, zoneNum), fGeoConfig->GetZoneThetaEnd(region, zoneNum));
		...
		if (fGeoConfig->GetUseOverallCoverage() || fGeoConfig->GetUseZonalCoverage()) {
			...
			maxNumCells = (int) (targetCoverage
					* WCSimPolygonTools::GetSliceAreaFromAngles(fGeoConfig->GetNSides(), fGeoConfig->GetOuterRadius(),
							...
							fGeoConfig->GetZoneThetaEnd(region, zoneNum)) / endcapCell.GetPhotocathodeArea());
...
//  double capPolygonOuterRadius = WCSimPolygonTools::GetOuterRadiusFromInner(fGeoConfig->GetNSides(), fCapPolygonCentreRadius);
	double capPolygonOuterRadius = WCSimPolygonTools::GetOuterRadiusFromInner(fGeoConfig->GetNSides(),
			fCapRingRadiusInside - 0.0001 * mm);
...
//	  std::cout << "Iteration " << iIter << ": Finished and updated cellSide to " << cellSide << std::endl;
	...
	if (fGeoConfig->GetLimitPMTNumber()) {
		std::cout << "bestNumCells = " << bestNumCells << " (maxNumCells = " << maxNumCells
				<< ") - chosen on iteration " << bestIter << " side = " << bestSide << std::endl << std::endl;
	} else {
...
double WCSimCherenkovBuilder::GetOptimalWallCellSize(int iZone) {
	...
	if (fGeoConfig->GetLimitPMTNumber()) {
		...
		std::cout << "Maximum coverage with available PMTs for region " << " kWall, zone " << iZone << " = "
				<< maxCoverage << std::endl;
...
//		std::cout << "Before casting: " << (wallSide / (this->GetWallUnitCell(iZone)->GetCellSizeForCoverage(coverage)));
		...
		for (int xIndex = 0; xIndex < 2; ++xIndex) {
			for (int zIndex = 0; zIndex < 2; ++zIndex) {
				...
				double newCoverage = wallCell->GetPhotocathodeCoverage(length) * (xNums[xIndex] * zNums[zIndex])
						* length * length / (wallSide * wallHeight);
...
void WCSimCherenkovBuilder::ConstructEndCapRings(G4int zflip) {
	...
	G4Polyhedra* solidEndCapRing = new G4Polyhedra("EndCapRing", 0. * deg, // phi start
	360.0 * deg, fGeoConfig->GetNSides(), //NPhi-gon
...
void WCSimCherenkovBuilder::ConstructEndCapSurfaces(G4int zflip) {
	...
	G4VSolid* solidWCCapEndBlacksheet = new G4Polyhedra("WCCapEndBlacksheet", 0. * deg, // phi start
	360.0 * deg, //total phi
	fGeoConfig->GetNSides(), //NPhi-gon
			2, //  z-planes
	...
	capBSPhysics = new G4PVPlacement(0, G4ThreeVector(0., 0., (0.5 * fCapPolygonEndBSHeight + 0.00001 * mm) * zflip),
	...
	G4VSolid* solidWCCapEndWhitesheet = new G4Polyhedra("WCCapEndWhitesheet", 0. * deg, // phi start
	360.0 * deg, //total phi
	fGeoConfig->GetNSides(), //NPhi-gon
	...
	capWSPhysics = new G4PVPlacement(0, G4ThreeVector(0., 0., (-0.5 * fCapPolygonEndWSHeight - 0.00001 * mm) * zflip),
...
//            std::cout << " PMT POSITION = " << PMTPosition.x() << ", " << PMTPosition.y() << ", " << PMTPosition.z() << " :: " << zflip << std::endl;
						...
						if
(						pmtFaceType.at(iPMT) != WCSimGeometryEnums::PMTDirection_t::kVeto) { // check PMT position for Plane Pipes only for non Veto PMT
...
void WCSimCherenkovBuilder::PlaceEndCapPlanePipes(G4int zflip, G4int zone) {
	...
	for (int xit = 0; xit < fEndCapExtremePMTPos_Max.size(); ++xit) {
		...
		G4Tubs *planePipeTube = new G4Tubs(name, 0, fPlanePipeRadius, pipeLength / 2, 0 * deg, 360 * deg);
		...
		if (xit < fEndCapExtremePMTPos_Max.size() - 1) {
			...
			G4Tubs *shortPlanePipeTube = new G4Tubs(name, 0, fPlanePipeRadius, shortPipeLength / 2, 0 * deg, 360 * deg);
	...
	for (int zit = 0; zit < fBarrelExtremePMTPos_Max.size(); ++zit) {
		...
		for (int sit = 0; sit < nPlanes; ++sit) {
			...
			G4ThreeVector shortShift1(0, shortPipeExcess / 2 + sit * fPlanePipeStep,
					shortPipeLength / 2 + fPlanePipeRadius + 0.1 * mm);
...
G4LogicalVolume * WCSimCherenkovBuilder::ConstructWC() {
	...
	WCSimGeoManager * manager = new WCSimGeoManager();
	fGeoConfig = new WCSimGeoConfig(manager->GetGeometryByName(fDetectorName));
	std::cout << "test->" << fDetectorName << "!!!" << std::endl;
...
void WCSimCherenkovBuilder::SetPositions() {
	...
	innerAnnulusRadius = fGeoConfig->GetOuterRadius() - fPMTConfigs[0].GetMaxExposeHeight() - 1. * mm;
	...
	WCRadius = (WCIDDiameter / 2. + fBlacksheetThickness + 1.5 * m) / cos(dPhi / 2.);
	...
	G4cout << "Computed tolerance = " << G4GeometryTolerance::GetInstance()->GetSurfaceTolerance() / mm << " mm"
			<< G4endl;
