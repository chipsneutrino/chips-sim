...
WCSimGeantPMTWrapper::WCSimGeantPMTWrapper() :
		fPMTLogicalVolume(NULL), fGlassFaceLogicalVolume(NULL) {
}

WCSimGeantPMTWrapper::WCSimGeantPMTWrapper(G4LogicalVolume* PMTLogicalVolume, G4LogicalVolume* glassFaceLogicalVolume) :
		fPMTLogicalVolume(PMTLogicalVolume), fGlassFaceLogicalVolume(glassFaceLogicalVolume) {
}
...
void WCSimPMTBuilder::ConstructPMT(WCSimPMTConfig config) {
	...
	G4Polycone* solidWCPMT = new G4Polycone(("WCPMT_" + pmtName).c_str(), 0.0 * deg, 360.0 * deg, 2, PMTHolderZ,
...
//		  	  	  	  	  	  	  	  	    PMTHolderZ, PMTHolderR, PMTHolderr);
	...
	//Create PMT Interior
	G4Sphere* tmpSolidInteriorWCPMT = new G4Sphere("tmpInteriorWCPMT", 0.0 * m,
			(sphereRadius - config.GetGlassThickness()), 0.0 * deg, 360.0 * deg, 0.0 * deg, 90.0 * deg);
	...
	G4Sphere* tmpGlassFaceWCPMT = new G4Sphere("tmpGlassFaceWCPMT", (sphereRadius - config.GetGlassThickness()),
			sphereRadius, 0.0 * deg, 360.0 * deg, 0.0 * deg, 90.0 * deg);
	...
	if (unsensitiveHeight > 0.1 * cm) {
		...
		G4VPhysicalVolume* physiPMTBack = new G4PVPlacement(0, G4ThreeVector(0, 0, unsensitiveHeight - 0.05 * cm),
	...
	if (lcPoints > 0) {
		G4Polycone* solidWCLCSupport = new G4Polycone(("LightCollectorSupport" + pmtName).c_str(), 0.0 * deg,
				360.0 * deg, 100, LCZ, LCrb, LCR);
		...
		G4Polycone* solidWCLC = new G4Polycone(("LightCollector" + pmtName).c_str(), 0.0 * deg, 360.0 * deg, 100, LCZ,
				LCr, LCrb);
