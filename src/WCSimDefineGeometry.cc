...
void WCSimDetectorConstruction::SetSuperKGeometry()
{
	ResetPMTConfigs();
	//  WCPMTName             ="20inch";
	//  WCPMTRadius           =.254*CLHEP::m;
	//  WCPMTExposeHeight     =.18*CLHEP::m;
	fPMTConfigs.push_back(fPMTManager->GetPMTByName("20inch"));
	WCIDDiameter = 33.6815 * CLHEP::m;	   //16.900*2*cos(2*CLHEP::pi*CLHEP::rad/75)*CLHEP::m; //inner detector diameter
	WCIDHeight = 36.200 * CLHEP::m;		   //"" "" height
	WCBarrelPMTOffset = 0.0715 * CLHEP::m; //offset from vertical
	WCBarrelNumPMTHorizontal = 150;
	WCBarrelNRings = 17.;
	WCPMTperCellHorizontal = 4;
	WCPMTperCellVertical = 3;
	WCCapPMTSpacing = 0.707 * CLHEP::m; // distance between centers of top and bottom pmts
	WCCapEdgeLimit = 16.9 * CLHEP::m;
	//  WCPMTGlassThickness   = .4*CLHEP::cm;
	WCBlackSheetThickness = 2.0 * CLHEP::cm;
	WCAddGd = false;
}
...
{
	//  WCPMTName             = "10inch";
	//  WCPMTRadius           = .127*CLHEP::m;
	//  WCPMTExposeHeight	    = WCPMTRadius - 0.01*CLHEP::m;
	ResetPMTConfigs();
	fPMTConfigs.push_back(fPMTManager->GetPMTByName("10inch"));
	WCIDDiameter = 53.0 * CLHEP::m;
	WCIDHeight = 60.0 * CLHEP::m;
	...
	WCBarrelNumPMTHorizontal = round(
		WCIDDiameter * sqrt(CLHEP::pi * WCPMTPercentCoverage) / (10.0 * fPMTConfigs[0].GetRadius()));
	WCBarrelNRings = round(
		((WCBarrelNumPMTHorizontal * ((WCIDHeight - 2 * WCBarrelPMTOffset) / (CLHEP::pi * WCIDDiameter))) / WCPMTperCellVertical));
	WCCapPMTSpacing = (CLHEP::pi * WCIDDiameter / WCBarrelNumPMTHorizontal);
	WCCapEdgeLimit = WCIDDiameter / 2.0 - fPMTConfigs[0].GetRadius();
	//  WCPMTGlassThickness   = .55*CLHEP::cm;
	WCBlackSheetThickness = 2.0 * CLHEP::cm;
	WCAddGd = false;
}
...
void WCSimDetectorConstruction::DUSEL_200kton_10inch_HQE_12perCent()
{
	...
	ResetPMTConfigs();
	fPMTConfigs.push_back(fPMTManager->GetPMTByName("10inch_HQE"));
	WCIDDiameter = 62.21 * CLHEP::m;
	WCIDHeight = 79.96 * CLHEP::m;
	//	WCBarrelPMTOffset	    = WCPMTRadius;
	WCBarrelPMTOffset = fPMTConfigs[0].GetRadius();
	WCPMTperCellHorizontal = 4.0;
	WCPMTperCellVertical = 3.0;
...
void WCSimDetectorConstruction::DUSEL_200kton_12inch_HQE_10perCent()
{
	...
	//	WCPMTRadius           = .1524*CLHEP::m;
	//	WCPMTExposeHeight	    = .118*CLHEP::m;
	ResetPMTConfigs();
	fPMTConfigs.push_back(fPMTManager->GetPMTByName("12inchHQE"));
	WCIDDiameter = 63.30 * CLHEP::m;
	WCIDHeight = 76.60 * CLHEP::m;
	WCBarrelPMTOffset = .1537 * CLHEP::m;
	WCPMTperCellHorizontal = 1.0;
	WCPMTperCellVertical = 1.0;
	WCBarrelNumPMTHorizontal = 232;
	WCBarrelNRings = 89;
	WCCapPMTSpacing = .8572 * CLHEP::m;
	WCCapEdgeLimit = 31.424 * CLHEP::m;
	//	WCPMTGlassThickness   = .55*CLHEP::cm; //guess
	WCBlackSheetThickness = 2.0 * CLHEP::cm; //excess, should be just as light-tight
	WCAddGd = false;
}
...
void WCSimDetectorConstruction::DUSEL_200kton_12inch_HQE_14perCent()
{
	...
	WCIDDiameter = 63.30 * CLHEP::m;
	WCIDHeight = 76.60 * CLHEP::m;
	WCBarrelPMTOffset = .1951 * CLHEP::m;
	WCPMTperCellHorizontal = 1.0;
	WCPMTperCellVertical = 1.0;
	WCBarrelNumPMTHorizontal = 274;
	WCBarrelNRings = 105;
	WCCapPMTSpacing = .7258 * CLHEP::m;
	WCCapEdgeLimit = 31.624 * CLHEP::m;
	//	WCPMTGlassThickness   = .55*CLHEP::cm; //guess
	WCBlackSheetThickness = 2.0 * CLHEP::cm; //excess, should be just as light-tight
	WCAddGd = false;
}
...
void WCSimDetectorConstruction::CHIPS_25kton_10inch_HQE_10perCent()
{
	...
	WCIDDiameter = 40.0 * CLHEP::m;
	WCIDHeight = 20.0 * CLHEP::m;
	//  WCBarrelPMTOffset	    = WCPMTRadius;
	WCBarrelPMTOffset = fPMTConfigs[0].GetRadius();
	WCPMTperCellHorizontal = 4.0;
	WCPMTperCellVertical = 3.0;
...
void WCSimDetectorConstruction::GiantPhotonTest()
{
	...
	WCIDDiameter = 2000.0 * CLHEP::m;
	WCIDHeight = 2000.0 * CLHEP::m;
	//  WCBarrelPMTOffset	    = WCPMTRadius;
	WCBarrelPMTOffset = fPMTConfigs[0].GetRadius();
	WCPMTperCellHorizontal = 4.0;
	WCPMTperCellVertical = 3.0;
...
void WCSimDetectorConstruction::SetMailBox100kTGeometry() // This should setup a 100kT (metric) Fiducial Volume
{
	...
	WC_MB_Tank_Airgap = 2.0 * CLHEP::m;		 //Arbitrary here, but this is just the height of air between the top of the outer Veto_thickness and the beginning of the Cavern Dome
	WC_MB_Dome_Height_fraction = 0.2;		 //fraction should be <=0.5
	WC_MB_Veto_Thickness = 2.0 * CLHEP::m;	 // This is just the spacing (in water) between the outer side of the Blacksheet and the beginning of the fiducial volume
	WC_MB_Buffer_Thickness = 2.5 * CLHEP::m; //Distance between wall and beginning of the Fiducial volume
	...
	WC_MB_Fid_Length = (45.3333 * CLHEP::m + 2 * WC_MB_Veto_Thickness); // strictly speaking this isn't the fiducial length anymore, but increased by the veto thickness
	WC_MB_Fid_Width = (40.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness);
	WC_MB_Fid_Depth = (55.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness); // how deep is the bottom layer of tubes at. Tank depth is a little more.
	//	WC_MB_Fid_Length          = 1.0*CLHEP::m+2*WC_MB_Veto_Thickness;// tiny test detector!
	//	WC_MB_Fid_Width           = 1.000*CLHEP::m+2*WC_MB_Veto_Thickness;// tiny test detector!
	//	WC_MB_Fid_Depth           = 1.0*CLHEP::m+2*WC_MB_Veto_Thickness;// tiny test detector!
	WCAddGd = false;
}
...
void WCSimDetectorConstruction::SetMailBox100x20x30Geometry() // This should setup a 60kT-ish (metric) Fiducial Volume
{
	...
	WC_MB_Buffer_Thickness = 2.5 * CLHEP::m; //Distance between wall and beginning of the Fiducial volume
	//Fiducial VOlume Stuff--these numbers are taken from table 9.1 of DUSEL LCAB Report #2, 7/2009, adjusted to Fiducial Volume (F.V.) and 2.5m gap between wall and F.V.
	WC_MB_Fid_Length = (100.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness); // strictly speaking this isn't the fiducial length anymore, but increased by the veto thickness
	WC_MB_Fid_Width = (20.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness);
	WC_MB_Fid_Depth = (30.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness); // how deep is the bottom layer of tubes at. Tank depth is a little more.
	//	WC_MB_Fid_Length          = 1.0*CLHEP::m+2*WC_MB_Veto_Thickness;// tiny test detector!
	//	WC_MB_Fid_Width           = 1.000*CLHEP::m+2*WC_MB_Veto_Thickness;// tiny test detector!
	//	WC_MB_Fid_Depth           = 1.0*CLHEP::m+2*WC_MB_Veto_Thickness;// tiny test detector!
	WCAddGd = false;
...
void WCSimDetectorConstruction::SetMailBox150kTGeometry_10inch_HQE_30perCent() // This should setup a 150kT (metric) Fiducial Volume
{
	...
	WC_MB_Buffer_Thickness = 2.5 * CLHEP::m; //Distance between wall and beginning of the Fiducial volume
	//Fiducial VOlume Stuff--these numbers are taken from table 9.1 of DUSEL LCAB Report #2, 7/2009, adjusted to Fiducial Volume (F.V.) and 2.5m gap between wall and F.V.
	WC_MB_Fid_Length = (68.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness); // strictly speaking this isn't the fiducial length anymore, but increased by the veto thickness
	WC_MB_Fid_Width = (40.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness);
	WC_MB_Fid_Depth = (55.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness); // how deep is the bottom layer of tubes at. Tank depth is a little more.
	WCAddGd = false;
}
...
void WCSimDetectorConstruction::SetMailBox300kTGeometry() // This should setup a 300kT (metric) Fiducial Volume
{
	...
	WC_MB_Buffer_Thickness = 2.5 * CLHEP::m; //Distance between wall and beginning of the Fiducial volume
	//Fiducial VOlume Stuff--these numbers are taken from table 9.1 of DUSEL LCAB Report #2, 7/2009, adjusted to Fiducial Volume (F.V.) and 2.5m gap between wall and F.V.
	WC_MB_Fid_Length = (136.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness); //	// strictly speaking this isn't the fiducial length anymore, but increased by the veto thickness
	WC_MB_Fid_Width = (40.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness);	  // this gives a volume of 299.2 metric tons
	WC_MB_Fid_Depth = (55.0 * CLHEP::m + 2 * WC_MB_Veto_Thickness);	  // how deep is the bottom layer of tubes at. Tank depth is a little more.
	WCAddGd = false;
}
//aah
