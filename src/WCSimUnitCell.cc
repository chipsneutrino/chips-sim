...
WCSimPMTPlacement::WCSimPMTPlacement(WCSimPMTConfig pmt, double x, double y) : fPMTConfig(pmt), fX(x), fY(y)
{
...
WCSimPMTPlacement::WCSimPMTPlacement(WCSimPMTConfig pmt, double x, double y, WCSimGeometryEnums::PMTDirection_t dir,
									 double theta, double phi) : fPMTConfig(pmt), fX(x), fY(y), fDir(dir), fTheta(theta), fPhi(phi)
{
}
...
{
	std::cout << "-------------------------" << std::endl
			  << "PMT of type " << fPMTConfig.GetPMTName() << " at ("
			  << fX / CLHEP::m << "," << fY / CLHEP::m << "):" << std::endl;
	fPMTConfig.Print();
	std::cout << "-------------------------" << std::endl
			  << std::endl;
}
...
G4TwoVector WCSimUnitCell::GetPMTPos(unsigned int pmt, double side) const
{
	...
	return G4TwoVector(side / CLHEP::m * fPMTs.at(pmt).GetX(), side / CLHEP::m * fPMTs.at(pmt).GetY());
}
...
void WCSimUnitCell::Print() const
{
	...
	{
		std::cout << (*itr).GetPMTConfig().GetPMTName() << " at (" << (*itr).GetX() / CLHEP::m << "," << (*itr).GetY() / CLHEP::m
				  << ")" << std::endl;
		if ((*itr).GetDir() != WCSimGeometryEnums::PMTDirection_t::kInwards)
			std::cout << " Dir " << (*itr).GetDir() << " Angles (" << (*itr).GetTheta() * CLHEP::deg << ","
					  << (*itr).GetPhi() * CLHEP::deg << ")" << std::endl;
	}
	std::cout << "-----------------" << std::endl
			  << std::endl;
}
...
{
	//  std::cout << "x = " << x << " y = " << y << " 1m = " << 1*CLHEP::m  << std::endl;
	assert(0.0 * CLHEP::m < x && "x position must be greater than 0 (and < 1)");
	assert(1.0 * CLHEP::m > x && "x position must be less than 1 (and > 0)");
	assert(0.0 * CLHEP::m < y && "y position must be greater than 0 (and < 1)");
	assert(1.0 * CLHEP::m > y && "y position must be less than 1 (and > 0)");
...
double WCSimUnitCell::GetMinimumCellSize() const
{
	...
	for (std::vector<WCSimPMTPlacement>::const_iterator itr = fPMTs.begin(); itr != fPMTs.end(); ++itr)
	{
		...
		double scaleFacs[4] = {radius / x, radius / (1 * CLHEP::m - x), radius / y, radius / (1 * CLHEP::m - y)};
	...
	{
		return closenessScale * CLHEP::m + 1 * CLHEP::mm;
	}
	return scale * CLHEP::m + 1 * CLHEP::mm;
}
