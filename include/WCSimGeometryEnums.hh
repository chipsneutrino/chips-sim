...
struct PhotodetectorLimit_t {
		enum Type {
			kUnknown = 0, kPercentCoverage = 1, kTotalNumber = 2, kZonalCoverage = 3
		};
		Type fType;

		PhotodetectorLimit_t(Type t = kUnknown) :
				fType(t) {
		}
		PhotodetectorLimit_t(const PhotodetectorLimit_t &ep) :
				fType(ep.fType) {
		}
		;
		PhotodetectorLimit_t(const std::string typeName) :
				fType(kUnknown) {
			if (typeName == "kUnknown") {
				fType = kUnknown;
			} else if (typeName == "kPercentCoverage") {
				fType = kPercentCoverage;
			} else if (typeName == "kTotalNumber") {
				fType = kTotalNumber;
			} else if (typeName == "kZonalCoverage") {
				fType = kZonalCoverage;
			} else {
				fType = kUnknown;
			}
			if (fType == kUnknown) {
		...
		static std::string AsString(Type type) {
			...
			if (type == kPercentCoverage) {
				str = "kPercentCoverage";
			}
			if (type == kTotalNumber) {
				str = "kTotalNumber";
			}
			if (type == kZonalCoverage) {
				str = "kZonalCoverage";
			}
			return str;
		}
...
struct DetectorRegion_t {
		enum Type {
			kUnknown = 0, kWall = 1, kTop = 2, kBottom = 3
		};
		Type fType;

		DetectorRegion_t(Type t = kUnknown) :
				fType(t) {
		}
		DetectorRegion_t(const DetectorRegion_t &ep) :
				fType(ep.fType) {
		}
		;
		DetectorRegion_t(const std::string typeName) :
				fType(kUnknown) {
			if (typeName == "kWall") {
				fType = kWall;
			} else if (typeName == "kTop") {
				fType = kTop;
			} else if (typeName == "kBottom") {
				fType = kBottom;
			} else {
				fType = kUnknown;
			}
...
struct PMTDirection_t {
		...
		PMTDirection_t(Type t = kUnknown) :
				fType(t) {
		}
		PMTDirection_t(const PMTDirection_t &ep) :
				fType(ep.fType) {
		}
		;
		operator Type() const {
		...
		static std::string AsString(Type type) {
			...
			if (type == kAngledUpstream) {
				str = "kAngledUpstream";
			}
			if (type == kAngledDownstream) {
				str = "kAngledDownstream";
			}
			if (type == kArbitrary) {
				str = "kArbitrary";
			}
		...
		static PMTDirection_t FromString(std::string string) {
			...
			if (string == "kAngledDown") {
				type = kAngledDown;
			}
			if (string == "kAngledUpstream") {
				type = kAngledUpstream;
			}
			if (string == "kAngledDownstream") {
				type = kAngledDownstream;
			}
			if (string == "kArbitrary") {
				type = kArbitrary;
			}
			if (string == "kVeto") {
				type = kVeto;
			...
			if (type == kUnknown) {
				std::cerr << "Error: unknown PMT direction type \"" << string << "\"" << std::endl;
				assert(0);
			}
...
struct PMTSupport_t {
		enum Type {
			kUnknown = 0, kNone = 1, kPOM = 2
		};
		Type fType;

		PMTSupport_t(Type t = kUnknown) :
				fType(t) {
		}
		PMTSupport_t(const PMTSupport_t &ep) :
				fType(ep.fType) {
		}
		;
		PMTSupport_t(const std::string typeName) :
				fType(kUnknown) {

			if (typeName == "kUnknown") {
				fType = kUnknown;
			} else if (typeName == "kNone") {
				fType = kNone;
			} else if (typeName == "kPOM") {
				fType = kPOM;
			} else {
				fType = kUnknown;
			}
		...
		static PMTSupport_t FromString(std::string string) {
			...
			if (type == kUnknown) {
				std::cerr << "Error: unknown PMT support type \"" << string << "\"" << std::endl;
				assert(0);
			}
