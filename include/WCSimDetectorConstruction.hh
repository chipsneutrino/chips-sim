...
//  G4String GetPMTName()			  {return WCPMTName;}
		...
		G4int GetNumVetoPmts() {
			return fVetoPMTs;
		}

		G4int GetPMT_QE_Method() {
			return PMT_QE_Method;
		}
		...
		static G4Transform3D GetTubeTransform(int tubeNo) {
			return tubeIDMap[tubeNo];
		}

		// Related to Pi0 analysis
		G4bool SavePi0Info() {
			return pi0Info_isSaved;
		}
		void SavePi0Info(G4bool choice) {
			pi0Info_isSaved = choice;
		}
		...
		G4int GetPMTSim() const {
			return PMTSim_Method;
		}

		void SetPMTSim(G4int val) {
			PMTSim_Method = val;
		}

		// Andy: getters and setters for the PMT timing options
		G4int GetPMTTime() const {
			return PMTTime_Method;
		}

		void SetPMTTime(const G4int &val) {
			PMTTime_Method = val;
		}

		G4bool GetPMTPerfectTiming() const {
			return PMTPerfectTiming;
		}

		void SetPMTPerfectTiming(const G4bool &val) {
			PMTPerfectTiming = val;
		}
		...
		void SetIsMailbox(G4bool choice) {
			isMailbox = choice;
		}
		G4bool GetIsMailbox() {
			return isMailbox;
	...
	protected:
		// Changed this from private to let WCSimCherenkovBuilder
		...
		typedef void (WCSimDetectorConstruction::*DescriptionFcnPtr)(G4VPhysicalVolume*, int, int,
				const G4Transform3D&);
