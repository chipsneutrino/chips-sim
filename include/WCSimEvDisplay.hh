...
public:
	...
	void SetPi0Information(double en, TVector3 vtx, TVector3 dir)
	{
		fPi0Energy = en;
		fPi0Vtx = vtx;
		fPi0Dir = dir;
	...
	double GetPi0Energy() const
	{
		return fPi0Energy;
	}

	TVector3 GetPi0Vertex() const
	{
		return fPi0Vtx;
	}

	TVector3 GetPi0Direction() const
	{
		return fPi0Dir;
	}
	...
	{
		if (id == 1)
			return fPhotonEnergy1;
		else if (id == 2)
			return fPhotonEnergy2;
		else
		{
			std::cerr << "Photon id must be either 1 or 2" << std::endl;
			return 0.;
	...
	void Print() const
	{
		...
		std::cout << "  - Direction = " << fPhotonDir1.X() << "," << fPhotonDir1.Y() << "," << fPhotonDir1.Z()
				  << std::endl;
		std::cout << " = Decay Photon: " << std::endl;
		std::cout << "  - Energy    = " << fPhotonEnergy2 << std::endl;
		std::cout << "  - Direction = " << fPhotonDir2.X() << "," << fPhotonDir2.Y() << "," << fPhotonDir2.Z()
				  << std::endl;
	}
...
protected:
	...
	void MakePolyMarker(std::vector<double> coord1, std::vector<double> coord2, std::vector<TPolyMarker *> &poly,
						int colour);
