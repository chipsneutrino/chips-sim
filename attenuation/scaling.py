...
def main():
    ...
    parser.add_argument('-m', '--mie-weight', default=-1, type=float,
            help='Relative weight for Mie scattering'
            '(off when negative, default -1)')
    ...
    if use_mie:
        scale_abs, scale_scat, scale_mie = get_scale_factors(
                args.att_length, args.wavelength, args.abs_weight,
                args.scatt_weight, args.mie_weight)
    else:
...
def get_scale_factors(att_length, wavelength, abs_weight=1.,
                        scat_weight=1., mie_weight=-1):
    # get base attenuation give relative weights
    att_base = get_attenuation_length(wavelength, abs_weight,
                                        scat_weight, mie_weight)
...
def get_attenuation_length(wavelength, scale_abs, scale_scat, scale_mie):
    ...
    (photon_e_array, abs_length_array,
            scat_length_array, mie_length_array) = wcsim_data
...
def run_root_macro(scale_abs, scale_scat, scale_mie):
    os.system('root -l "attenuation.C(%g, %g, %g, true)"' % 
            (scale_abs, scale_scat, scale_mie))
