...
void simplots(
    const char *in_dir = "./",
    const char *out_name = "plots.root",
    int pdg = 13,
    bool all_plots = true,
    ...
    double d_height = 1200)
{
            ...
            for (int evt = 0; evt < nevent; evt++)
            {
                ...
                for (int p = 0; p < truthSum.GetNPrimaries(); p++)
                {
                    ...
                    if (code == pdg)
                    {
                        ...
                        if (dir.X() > 0 && dir.Y() < 0)
                        {
                            dirPhi = TMath::ATan(dir.Y() / dir.X());
                        }
                        else if (dir.X() < 0 && dir.Y() < 0)
                        {
                            dirPhi = TMath::ATan(dir.Y() / dir.X()) - TMath::Pi();
                        }
                        else if (dir.X() < 0 && dir.Y() > 0)
                        {
                            dirPhi = TMath::ATan(dir.Y() / dir.X()) + TMath::Pi();
                        ...
                        {
                            std::cout << "Error: Can't find barrel phi dir angle!" << std::endl;
                        }
                ...
                for (int h = 0; h < ncherenkovdigihits; h++)
                {
                    ...
                    if (all_plots)
                    {
                        ...
                        else
                        {
                            ...
                            if (hitX > 0 && hitY < 0)
                            {
                                hitPhi = TMath::ATan(hitY / hitX);
                            }
                            if (hitX < 0 && hitY < 0)
                            {
                                hitPhi = TMath::ATan(hitY / hitX) - TMath::Pi();
                            }
                            if (hitX < 0 && hitY > 0)
                            {
                                hitPhi = TMath::ATan(hitY / hitX) + TMath::Pi();
    ...
    if (hVtxX->GetEntries() > 0)
    {
        hVtxX->Scale(1 / (hVtxX->GetEntries()));
    }
    if (hVtxY->GetEntries() > 0)
    {
        hVtxY->Scale(1 / (hVtxY->GetEntries()));
    }
    if (hVtxZ->GetEntries() > 0)
    {
        hVtxZ->Scale(1 / (hVtxZ->GetEntries()));
    }
    if (hVtxT->GetEntries() > 0)
    {
        hVtxT->Scale(1 / (hVtxT->GetEntries()));
    }
    if (hDirX->GetEntries() > 0)
    {
        hDirX->Scale(1 / (hDirX->GetEntries()));
    }
    if (hDirY->GetEntries() > 0)
    {
        hDirY->Scale(1 / (hDirY->GetEntries()));
    }
    if (hDirZ->GetEntries() > 0)
    {
        hDirZ->Scale(1 / (hDirZ->GetEntries()));
    }
    if (hDirTheta->GetEntries() > 0)
    {
        hDirTheta->Scale(1 / (hDirTheta->GetEntries()));
    }
    if (hDirPhi->GetEntries() > 0)
    {
        hDirPhi->Scale(1 / (hDirPhi->GetEntries()));
    }
    if (hLepEnergy->GetEntries() > 0)
    {
        hLepEnergy->Scale(1 / (hLepEnergy->GetEntries()));
    }

    if (hHits->GetEntries() > 0)
    {
        hHits->Scale(1 / (hHits->GetEntries()));
    }
    if (hDigiHits->GetEntries() > 0)
    {
        hDigiHits->Scale(1 / (hDigiHits->GetEntries()));
    }
    if (hCharge->GetEntries() > 0)
    {
        hCharge->Scale(1 / (hCharge->GetEntries()));
    }

    if (hpdg->GetEntries() > 0)
    {
        hpdg->Scale(1 / (hpdg->GetEntries()));
    }

    if (all_plots)
    {
        if (hHitX->GetEntries() > 0)
        {
            hHitX->Scale(1 / (hHitX->GetEntries()));
        }
        if (hHitY->GetEntries() > 0)
        {
            hHitY->Scale(1 / (hHitY->GetEntries()));
        }
        if (hHitZ->GetEntries() > 0)
        {
            hHitZ->Scale(1 / (hHitZ->GetEntries()));
        }
        if (hHitBarrel->GetEntries() > 0)
        {
            hHitBarrel->Scale(1 / (hHitBarrel->GetEntries()));
        }
        if (hHitEndcap->GetEntries() > 0)
        {
            hHitEndcap->Scale(1 / (hHitEndcap->GetEntries()));
        }
    }
